...
public:
    ...
    TDrawingItem(AnsiString S);//文字列からデコード
    __fastcall virtual ~TDrawingItem();

    AnsiString Encode();//文字列で記述
private:
...
public:
    ...
    TDrawing(AnsiString S);//文字列からデコード
    __fastcall virtual ~TDrawing();

    AnsiString Encode(bool bAll = false);//文字列で記述。bAll=falseで選択されているアイテムのみ。bAll=trueですべて
private:
