...
void TCard::CheckImageFN()
{
    ...
    for (int i = 0 ; i < m_Lines->Count ; i++){
		if (IsFileNameOrURL(m_Lines->Strings[i])){
            AnsiString Ext = LowerCase(ExtractFileExt(m_Lines->Strings[i]));
...
void TCard::LoadFromString(TStringList *SL, int &line, int version)
{
    ...
    while (m_Lines->Count){
        AnsiString S = m_Lines->Strings[0];
        m_Lines->Delete(0);
...
void TCard::SaveToString(TStringList *SL)
{
    ...
    SL2->Insert(0, "-");
    //ヘッダ
    SL2->Insert(0, AnsiString("Viewed:") + DateTimeToStr(m_fViewed));
    SL2->Insert(0, AnsiString("Updated:") + DateTimeToStr(m_fUpdated));
    SL2->Insert(0, AnsiString("Created:") + DateTimeToStr(m_fCreated));

    SL2->Insert(0, AnsiString("Visible:") + IntToStr(m_bVisible));
    SL2->Insert(0, AnsiString("Shape:") + IntToStr(m_nShape));
    SL2->Insert(0, AnsiString("Size:") + IntToStr(m_nSize));
	if (m_bFold){
		SL2->Insert(0, AnsiString("Fold:") + IntToStr(m_bFold));
	}
	if (m_bTop){
		SL2->Insert(0, AnsiString("Top:") + IntToStr(m_bTop));
	}
	SL2->Insert(0, AnsiString("Y:") + FloatToStr(m_fY));
	SL2->Insert(0, AnsiString("X:") + FloatToStr(m_fX));
	SL2->Insert(0, AnsiString("Fixed:") + IntToStr(m_bFixed));

	if (m_Drawing->Count) {
		SL2->Insert(0, AnsiString("Drawing:") + m_Drawing->Encode(true));
	}
	if (m_Labels->Count) {
		SL2->Insert(0, AnsiString("Label:") + m_Labels->Encode());
	}
	SL2->Insert(0, AnsiString("Title:") + m_Title);
...
//---------------------------------------------------------------------------
void TCard::LoadFromFile(AnsiString FN)
{
	//互換用
...
//---------------------------------------------------------------------------
void TCard::SaveToFile(AnsiString FN)
{
	//互換用
	...
	SL->Insert(0, "-");
	//ヘッダ
	SL->Insert(0, AnsiString("Viewed:") + DateTimeToStr(m_fViewed));
	SL->Insert(0, AnsiString("Updated:") + DateTimeToStr(m_fUpdated));
	SL->Insert(0, AnsiString("Created:") + DateTimeToStr(m_fCreated));

	SL->Insert(0, AnsiString("Y:") + FloatToStr(m_fY));
	SL->Insert(0, AnsiString("X:") + FloatToStr(m_fX));

	SL->Insert(0, AnsiString("Label:") + m_Labels->Encode());

	SL->Insert(0, AnsiString("Title:") + m_Title);
