...
// ---------------------------------------------------------------------------
__fastcall TFo_Progress::TFo_Progress(TComponent *Owner) : TForm(Owner) {}
...
void __fastcall TFo_Progress::Ti_CheckTimer(TObject *Sender) {
  if (m_bFinished) {
    ...
    if (GetTickCount() - m_nStartTime > 500 &&
        PG_Progress->Position < PG_Progress->Max / 2 && Width == 0) {
      // Display
...
// ---------------------------------------------------------------------------
void __fastcall TFo_Progress::FormCreate(TObject *Sender) { fMax = 1.0f; }
