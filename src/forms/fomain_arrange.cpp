...
void TFo_Main::BrowserArrange_Link(int i, TCard *Card, float *CardX,
	...
	if (m_Document->LabelIsFold(Card)) {
		...
		if (count > 0.0f) {
			...
			Card->m_fY =
				(float)((Y / count) * ratio + CardY[i] * (1.0f - ratio));
			Norm[i] = true;
		}
	}
	else {
	...
	if (type != 204) {
		...
		for (int ic = m_Document->m_Cards->Count - 1; ic >= 0; ic--) {
			...
			if (m_CardVisible[i] && m_CardAssign[i] == i && Card->m_bFixed &&
				i != targetcardindex) {
				if (type % 100 == 0) {
					PrepareMatrixArrange_AssignToMatrix(i, tmatrix, minx, maxx,
						miny, maxy);
				}
				else {
					PrepareMatrixArrange_AssignToMatrix2(i, tmatrix, minx, maxx,
						miny, maxy);
				}
			}
		}
		...
		if (m_nTargetCard >= 0) {
			...
			if (i >= 0)
				if (m_CardVisible[i] && m_CardAssign[i] == i) {
					if (type % 100 == 0) {
						PrepareMatrixArrange_AssignToMatrix(i, tmatrix, minx,
							maxx, miny, maxy);
					}
					else {
						PrepareMatrixArrange_AssignToMatrix2(i, tmatrix, minx,
							maxx, miny, maxy);
					}
				}
		}
		...
		for (int i = m_Document->m_Cards->Count - 1; i >= 0; i--) {
			...
			if (m_CardVisible[i] && m_CardRelated[i] && m_CardAssign[i]
				== i && i != targetcardindex && !Card->m_bFixed) {
				if (type % 100 == 0) {
					PrepareMatrixArrange_AssignToMatrix(i, tmatrix, minx, maxx,
						miny, maxy);
				}
...
void TFo_Main::BrowserArrangeByTree(int type, float ratio) {
							...
							if (Card->m_nLevel == il - 1) {
								...
								if (LastRoot == Root) {
									...
									if (rightcount-- > 0) {
										...
										Card->m_fY =
										LastRoot->m_fY +
										(-rightheight * 0.5f + rightheightsum +
	...
	for (int i = 0; i < m_Document->m_Cards->Count; i++)
		if (m_CardVisible[i]) {
			TCard *Card = m_Document->GetCardByIndex(i);
			if (Card->m_bFixed || (m_bMDownBrowser >
				0 && Card->m_nID == m_nTargetCard)) {
				Card->m_fX = xbak[i];
				Card->m_fY = ybak[i];
			}
			else {
