...
void TFo_Main::RefreshRecent(UnicodeString FN)
{
    ...
    for (int i = 0 ; i < 10 ; i++){
        if (SettingFile.m_RecentFiles[i] != ""){
            if (i < 9){
                MI_RecentFiles[i]->Caption = UnicodeString("&") + IntToStr(i + 1) + " " + SettingFile.m_RecentFiles[i];
            }else{
                MI_RecentFiles[i]->Caption = UnicodeString("1&0 ") + SettingFile.m_RecentFiles[i];
            }
		...
		if (SettingFile.m_RecentFolders[i] != ""){
            if (i < 9){
                MI_RecentFolders[i]->Caption = UnicodeString("&") + IntToStr(i + 1) + " " + SettingFile.m_RecentFolders[i] + "...";
            }else{
                MI_RecentFolders[i]->Caption = UnicodeString("1&0 ") + SettingFile.m_RecentFolders[i] + "...";
            }
    ...
    */
    ...
    for (int i = 0 ; i < MAXLABELS ; i++){
		...
		Bu_Label[i]->SetBounds(
			590 + 112 * i, //Bu_Label0->Left + (Bu_Label0->Width + 8) * i,
			8, //Bu_Label0->Top,
			104, //Bu_Label0->Width,
			26 //Bu_Label0->Height
			);
		...
		Bu_LinkLabel[i]->SetBounds(
			590 + 112 * i, //Bu_LinkLabel0->Left + (Bu_LinkLabel0->Width + 8) * i,
			8, //Bu_LinkLabel0->Top,
...
void __fastcall TFo_Main::Ti_CheckTimer(TObject *Sender)
{
	...
	if (SelectionChanged || ListChanged || Fo_View->m_bRefreshRequest || m_UndoRedo->m_bChanged){
        ...
        if (m_nCurrentCard >= 0 && m_nCurrentCard == m_nTargetCard){
            ...
            if (Card){
                UnicodeString Text = Card->m_Lines->Text.SubString(1, Card->m_Lines->Text.Length() - 2);
                if (Text != RE_Edit->Text && Card->m_Lines->Text != RE_Edit->Text){
					...
					if (m_nNextCardID == m_nCurrentCard){
						int line2 = RE_Edit->Perform(EM_GETFIRSTVISIBLELINE, 0, (NativeInt)0);
						if (line2 > line){
							while (line2-- > line){
                ...
                if (Card2) if (Card2->m_Lines->Count > 0){
                    if (Card2->m_bVisible){
                        Str->Add(new UnicodeString(
                            UnicodeString("Title : ") + DecodeES(Card2->m_Title, " ") + UnicodeString("\r\n\r\n") +
    ...
    if (PC_Client->ActivePage == TS_Browser && (Application->Active || SettingView.m_bRedrawInBackground)/* && !m_bMDownBrowser*/){
        ...
        if (SettingFile.fepOperation){
			if (SettingFile.fepOperation(m_Document, UnicodeString("Browser_RedrawByTimer").c_str())){
                b = false;
            }
        }
        if (b){
        ...
        if (SearchKeyword != WideString("")){
            ...
            if (srbak & 0x40){
				...
				if (!(srbak & 0x100)) {
					...
					La_GlobalSearchCount->Caption = IntToStr(m_GlobalSearchResult->Count) + UnicodeString(" Hits");
            ...
            if (hImc){
                ...
                */
                ...
                if (err != IMM_ERROR_NODATA && err != IMM_ERROR_GENERAL && err > 0){
                    //M_File->Caption = buf;
					long DW = Ed_TitleB->Perform(EM_POSFROMCHAR, Ed_TitleB->SelStart, (NativeInt)0);
...
//---------------------------------------------------------------------------
void TFo_Main::TextEditBackupSub(UnicodeString Action, int CardID, int SelStart, int SelLength)
{
    if (!m_bDoNotBackup){
...
void TFo_Main::RefreshLaStatus()
{
    if (m_nCurrentCard >= 0){
        ...
        if (Card){
			...
			S += UnicodeString("X : ") + IntToStr(RE_Edit->SelStart - RE_Edit->Perform(EM_LINEINDEX, line, (NativeInt)0) + 1) + UnicodeString("  ");

			S +=
				MLText.Line + UnicodeString(" : ") + IntToStr(line + 1) +
				UnicodeString("/") + IntToStr(Card->m_Lines->Count) + UnicodeString("  ");
			S +=
				MLText.Len + UnicodeString(" : ") + IntToStr(WideString(RE_Edit->Text).SubString(1, RE_Edit->SelStart + 1).Length() - line * 2) +
				UnicodeString("/") + IntToStr(WideString(Card->m_Lines->Text).Length() - Card->m_Lines->Count * 2) + UnicodeString("  ");
			S +=
				MLText.Size + UnicodeString(" : ") + /*SizeToStr(RE_Edit->SelStart + 1) +
				UnicodeString("/") + */SizeToStr(Card->m_Lines->Text.Length()) + UnicodeString("  ");

			SB_Status->Panels->Items[0]->Text = S;

			S = "";
			//Date
			S += MLText.Created + UnicodeString(" : ") + DateTimeToStr(Card->m_fCreated) + UnicodeString("  ");
			S += MLText.Edited + UnicodeString(" : ") + DateTimeToStr(Card->m_fUpdated) + UnicodeString("  ");
			S += MLText.Viewed + UnicodeString(" : ") + DateTimeToStr(Card->m_fViewed) + UnicodeString("  ");
        ...
        if (Button == mbLeft){
            if (!Card->m_bLabelIsFold){
                ...
                if (Shift.Contains(ssShift) && Shift.Contains(ssCtrl)){
                    ...
                    bool b = true;
                    if (SettingFile.fepOperation){
						b = !SettingFile.fepOperation(m_Document, UnicodeString("Card_EditTitle").c_str());
					}
...
void __fastcall TFo_Main::PB_BrowserDblClick(TObject *Sender)
{
    ...
    //ダブルクリックのフック
    if (SettingFile.fepOperation){
		if (SettingFile.fepOperation(m_Document, UnicodeString("Card_DblClick").c_str())){
            return;
        }
    }
        ...
        if (!Card->m_bVisible && !(SearchRequest & 0x20)){
            ...
            if (SearchRequest & 0x2){
                ...
                if (foundat >= 0){
                    ...
                    RE_Edit->SelLength = SearchKeyword.Length();//UnicodeString(SearchKeyword).Length();
                    Ed_Title->SelLength = 0;
                    Found = true;
                    break;
                }
...
{
    BackupSub(MD_SortCard->Caption + UnicodeString(" ") + ((TMenuItem*)Sender)->Caption);
...
void __fastcall TFo_Main::MH_FrieveSiteClick(TObject *Sender)
{
    ...
    ShellExecute(Handle, NULL, UnicodeString(AppURL).c_str(), NULL, NULL,SW_SHOW);
}
//---------------------------------------------------------------------------
...
void __fastcall TFo_Main::MH_FIPSiteClick(TObject *Sender)
{
	...
	ShellExecute(Handle, NULL, UnicodeString(SoftURL).c_str(), NULL, NULL,SW_SHOW);
}
//---------------------------------------------------------------------------
...
void __fastcall TFo_Main::PL_CardPropertyClick(TObject *Sender)
{
    ...
    if (Card){
        ...
        //Title
        S +=
			MLText.Title + UnicodeString(" : ") + Card->m_Title;
		S += ret;
		...
		S += MLText.Size + UnicodeString(" : ") + SizeToStr(Card->m_Lines->Text.Length());
		S += ret;

		S += ret;

		//Date
		S += MLText.Created + UnicodeString(" : ") + DateTimeToStr(Card->m_fCreated);
		S += ret;
		...
		Application->MessageBox(UnicodeString(S).c_str(), UnicodeString(DeleteActionKey(PL_CardProperty->Caption)).c_str(), 0);
	}
}
//---------------------------------------------------------------------------
    ...
    if (SD_Html->Execute()){
        //index.html
        {
			...
			try{
				...
				ShowMessage(MLText.FailedToSave + SD_Html->FileName + UnicodeString("."));
			}
			delete SL;
        }
        ...
        //cards.html
        {
            ...
            SL->Add("<head>");
            SL->Add(
                UnicodeString("<title>") + DocumentTitle + "</title>");
            SL->Add("<link rel=StyleSheet href=\"./style.css\">");

            SL->Add("</head>");
            SL->Add("<body>");
            SL->Add(
                UnicodeString("<font size=+2>") + DocumentTitle +  "</font>");
            SL->Add("<hr>");
            for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
                TCard *Card = m_Document->GetCardByIndex(i);
                SL->Add(
                    UnicodeString("<a href=\"./") +
                    IntToStr(i + 1) +
                    ".html\" target=\"card\">" +
                    Card->m_Title +
                    "</a><br>");
			...
			try{
				...
				ShowMessage(MLText.FailedToSave + ExtractFilePath(SD_Html->FileName) + "cards.html" + UnicodeString("."));
			}
			delete SL;
		}
        ...
        for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
            ...
            SL->Add("<head>");
            SL->Add(
                UnicodeString("<title>") + DocumentTitle + Card->m_Title + "</title>");
            SL->Add("<link rel=StyleSheet href=\"./style.css\">");

            SL->Add("</head>");
            SL->Add("<body>");
            SL->Add(
                UnicodeString("<font size=+2>") + DocumentTitle + Card->m_Title + "</font>");

            //前のカード、次のカード
            SL->Add("<hr>");
            SL->Add(
                UnicodeString("<a href=\"./1.html\">[Top]</a> "));
            if (i > 0){
                SL->Add(
                    UnicodeString("<a href=\"./") +
                    IntToStr(i - 1 + 1) +
			...
			try{
				...
				ShowMessage(MLText.FailedToSave + ExtractFilePath(SD_Html->FileName) + IntToStr(i + 1) + ".html" + UnicodeString("."));
			}
			delete SL;
        }
    }
...
void __fastcall TFo_Main::Bu_TestClick(TObject *Sender)
{
    ...
    RE_Edit->Clear();
    RE_Edit->Lines->BeginUpdate();
	RE_Edit->Lines->Add(UnicodeString("Time elapsed : ") + IntToStr((int)(timeGetTime() - t)) + "ms");
    RE_Edit->Lines->Add(UnicodeString("Items : ") + IntToStr(TD->m_nMaxSN));
    RE_Edit->Lines->Add("");
    for (int i = 0 ; i < TD->m_nMaxCombi ; i++){
        RE_Edit->Lines->Add(UnicodeString("<<<<<  ") + IntToStr(i + 1) + "-gram  >>>>>");
        RE_Edit->Lines->Add("");
        WideString WS;
    ...
    */
    ...
    SM->Finalize(true);
	RE_Edit->Lines->Add(UnicodeString("SM : ") + IntToStr((int)(timeGetTime() - t)) + "ms  (Ex : " + IntToStr((int)(t2 - t)) + "ms  Fn : " + IntToStr((int)(timeGetTime() - t2)) + "ms)");
    ...
    RE_Edit->Lines->Add(UnicodeString("SM Items (after DeleteSameCol): ") + SM->m_Items->Count);
	RE_Edit->Lines->Add(UnicodeString("DM : ") + IntToStr((int)(timeGetTime() - t)) + "ms  (Cos : " + IntToStr((int)(t2 - t)) + "ms  Fn : " + IntToStr((int)(timeGetTime() - t2)) + "ms)");
...
void __fastcall TFo_Main::MFI_HierarchicalTextFileClick(TObject *Sender)
{
    //階層テキスト読み込み
    if (OD_Txt->Execute()){
        BackupSub(MF_Import->Caption + UnicodeString(" ") + MFI_HierarchicalTextFile->Caption);
...
void TFo_Main::AddTxtsInFolder(UnicodeString TopDir, TCard *Parent)
{
    ...
    if (!((UnicodeString(Data.cFileName) == ".") || (UnicodeString(Data.cFileName) == ".."))){
...
void __fastcall TFo_Main::MFI_TxtFilesinaFolderClick(TObject *Sender)
{
	...
	if (SelectDirectory(L"", L"", Dir)){
        BackupSub(MF_Import->Caption + UnicodeString(" ") + MFI_TxtFilesinaFolder->Caption);
...
void __fastcall TFo_Main::MFI_HierarchicalTextFile2Click(TObject *Sender)
{
    //階層テキスト読み込み
    if (OD_Txt->Execute()){
        BackupSub(MF_Import->Caption + UnicodeString(" ") + MFI_HierarchicalTextFile2->Caption);
    ...
    if (SB_EditorRelated->Down && LB_Link->ItemIndex >= 0){
        ...
        if (Link){
            ...
            if (foundat >= 1){
                ...
                Me_EditorRelated->SelLength = Key.Length();//UnicodeString(SearchKeyword).Length();
            }else{
...
//---------------------------------------------------------------------------
void TFo_Main::ExportHierarchicalText(TStringList *SL, int CurrentLevel, UnicodeString HChar,
	TCard *CurrentParent){
