...
// ---------------------------------------------------------------------------
__fastcall TFo_Shape::TFo_Shape(TComponent *Owner) : TForm(Owner) {}

// ---------------------------------------------------------------------------
void __fastcall TFo_Shape::FormDeactivate(TObject *Sender) { Close(); }
...
void __fastcall TFo_Shape::PBPaint(TObject *Sender) {
  ...
  for (int i = 0; i < IL_Shape->Count; i++) {
    ...
    C->Draw((itemwidth - BMP2->Width) / 2,
            (itemheight - BMP2->Height) / 2 + i * itemheight, BMP2);
...
// ---------------------------------------------------------------------------
void __fastcall TFo_Shape::FormCreate(TObject *Sender) { m_bSelected = false; }
// ---------------------------------------------------------------------------
...
void __fastcall TFo_Shape::PBMouseDown(TObject *Sender, TMouseButton Button,
