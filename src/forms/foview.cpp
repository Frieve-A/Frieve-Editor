...
void TFo_View::SettingToGUI() {
	...
	TB_SizeLimitation->Position =
		(int)(log(SettingView.m_nSizeLimitation / 100.0) * (8 / log(4)) +
		100.5) - 100;
	...
	for (int i = 0; i < m_Document->m_Labels[1]->Count; i++) {
		SG_LinkLabels->Cells[0][i + 2] =
			m_Document->GetLabelByIndex(1, i)->m_Name;
	}
...
void __fastcall TFo_View::TB_LinkLimitationChange(TObject *Sender) {
	...
	if (TB_LinkLimitation->Position < TB_LinkLimitation->Max) {
		La_Links->Caption = IntToStr(TB_LinkLimitation->Position) + " " +
			MLText.Links;
	}
	else {
		/*
	...
	if (SG->Cells[ACol][0] == MLText.Fold) {
		...
		if (ARow > 1) {
			...
			if (type == 0) {
				...
				SettingView.m_bNoLinkLabelVisible =
					!SettingView.m_bNoLinkLabelVisible;
			}
			changed = true;
		...
		if (ARow > 1) {
			if (Y >= r.top + 2 && Y < r.bottom - 2 && X >= r.left + 2 && X <
				r.right - 2) {
				int size =
...
void __fastcall TFo_View::SG_LabelsDrawCell(TObject *Sender, int ACol, int ARow,
	...
	if (State.Contains(gdFocused) && ARow > 0 && ACol > 0) {
		...
		Canvas->Brush->Style = bsSolid;
		if (ARow >= 2) {
			Canvas->Pen->Color = (TColor)m_Document->GetLabelByIndex(type,
				ARow - 2)->m_nColor;
			Canvas->Brush->Color =
				(TColor)HalfColor(m_Document->GetLabelByIndex(type,
				ARow - 2)->m_nColor, 0xffffff, 0.5f);
		}
		else {
	...
	if (ARow == 0 || ACol == 0) {
		...
		SG->Cells[ACol][0] == MLText.Show || SG->Cells[ACol][0] == MLText.Hide)
		|| ARow == 1) {
		int checked = -1;
		if (SG->Cells[ACol][0] == MLText.Fold) {
			if (ARow > 1) {
			...
			if (ARow > 1) {
				checked = m_Document->GetLabelByIndex(type, ARow - 2)
					->m_bEnable;
			}
		}
		else if (SG->Cells[ACol][0] == MLText.Show) {
		...
		// Size
		UnicodeString S =
			IntToStr((int)(log(m_Document->GetLabelByIndex(type,
			ARow - 2)->m_nSize / 100.0) * (8 / log(4)) + 100.5) - 100);
...
void __fastcall TFo_View::TB_SizeLimitationChange(TObject *Sender) {
	SettingView.m_nSizeLimitation =
		(int)(100 * exp(TB_SizeLimitation->Position / (8 / log(4))) +
		100) - 100;
