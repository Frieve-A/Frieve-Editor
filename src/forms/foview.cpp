      ...
      int ARow, TRect &Rect, TGridDrawState State)
{
    ...
    if (ARow == 0 || ACol == 0){
		...
		//Size
		UnicodeString S = IntToStr((int)(log(m_Document->GetLabelByIndex(type, ARow - 2)->m_nSize / 100.0) * (8 / log(4)) + 100.5) - 100);
		Canvas->TextOut((Rect.left + Rect.right - Canvas->TextWidth(S)) / 2, Rect.top+2, S);
	}
}
//---------------------------------------------------------------------------
