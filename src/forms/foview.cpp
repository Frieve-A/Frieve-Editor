...
void TFo_View::ApplyLanguageSetting() {
  ...
  La_LinkLabelShow->Caption =
      Ini->ReadString("View", "La_Show",
                      La_LinkLabelShow->Caption); // Show:
  La_LinkLabelHide->Caption =
      Ini->ReadString("View", "La_Hide",
                      La_LinkLabelHide->Caption); // Hide:
  TS_Limitation->Caption =
      Ini->ReadString("View", "TS_Limitation",
                      TS_Limitation->Caption); // Display limit
  Ch_SizeLimitation->Caption =
      Ini->ReadString("View", "Ch_SizeLimitation",
                      Ch_SizeLimitation->Caption); // Limit by size
  La_Size->Caption = Ini->ReadString("View", "La_Size",
                                     La_Size->Caption); // Size
  Ch_LinkLimitation->Caption =
      Ini->ReadString("View", "Ch_LinkLimitation",
  ...
  Ch_LinkDirection->Caption =
      Ini->ReadString("View", "Ch_LinkDirection",
  ...
  RB_LinkBackward->Caption =
      Ini->ReadString("View", "RB_LinkBackward",
                      RB_LinkBackward->Caption); // Backward
  La_LinkTarget->Caption =
      Ini->ReadString("View", "La_LinkTarget",
                      La_LinkTarget->Caption); // Link target:
  Ch_DateLimitation->Caption =
      Ini->ReadString("View", "Ch_DateLimitation",
  ...
  PT_NoAssign->Caption =
      Ini->ReadString("View", "PT_NoAssign", PT_NoAssign->Caption);
...
// ---------------------------------------------------------------------------
__fastcall TFo_View::TFo_View(TComponent *Owner) : TForm(Owner) {}

// ---------------------------------------------------------------------------
void __fastcall TFo_View::FormDeactivate(TObject *Sender) { Close(); }
...
void TFo_View::SettingToGUI() {
  ...
  TB_SizeLimitation->Position =
      (int)(log(SettingView.m_nSizeLimitation / 100.0) * (8 / log(4)) + 100.5) -
      100;
...
void __fastcall TFo_View::SG_LabelsMouseDown(TObject *Sender,
  ...
  if (SG->Cells[ACol][0] == MLText.Fold) {
    if (ARow > 1) {
      m_Document->GetLabelByIndex(type, ARow - 2)->m_bFold =
          !m_Document->GetLabelByIndex(type, ARow - 2)->m_bFold;
      changed = true;
    }
  } else if (SG->Cells[ACol][0] == MLText.Enable) {
    if (ARow > 1) {
      m_Document->GetLabelByIndex(type, ARow - 2)->m_bEnable =
          !m_Document->GetLabelByIndex(type, ARow - 2)->m_bEnable;
      changed = true;
    }
  } else if (SG->Cells[ACol][0] == MLText.Show) {
    if (ARow > 1) {
      m_Document->GetLabelByIndex(type, ARow - 2)->m_bShow =
          !m_Document->GetLabelByIndex(type, ARow - 2)->m_bShow;
      changed = true;
    } else {
      if (type == 0) {
    ...
    if (ARow > 1) {
      m_Document->GetLabelByIndex(type, ARow - 2)->m_bHide =
          !m_Document->GetLabelByIndex(type, ARow - 2)->m_bHide;
      changed = true;
    }
  } else if (SG->Cells[ACol][0] == MLText.Size) {
    if (ARow > 1) {
      if (Y >= r.top + 2 && Y < r.bottom - 2 && X >= r.left + 2 &&
          X < r.right - 2) {
        int size = (int)(log(m_Document->GetLabelByIndex(type,
                                                         ARow - 2)
                                 ->m_nSize /
                             100.0) *
                             (8 / log(4)) +
        ...
        if (X < r.left + 13) {
          ...
          if (size > -8) {
            size--;
            m_Document->GetLabelByIndex(type, ARow - 2)->m_nSize =
                (int)(100 * exp(size / (8 / log(4))) + 100) - 100;
            changed = true;
          }
...
void __fastcall TFo_View::SG_LabelsDrawCell(TObject *Sender, int ACol, int ARow,
  ...
  if (State.Contains(gdFocused) && ARow > 0 && ACol > 0) {
    ...
    if (ARow >= 2) {
      Canvas->Pen->Color =
          (TColor)m_Document->GetLabelByIndex(type, ARow - 2)->m_nColor;
      Canvas->Brush->Color = (TColor)HalfColor(
          m_Document->GetLabelByIndex(type, ARow - 2)->m_nColor, 0xffffff,
          0.5f);
    } else {
    ...
    Canvas->Rectangle(Rect);
    // Canvas->Rectangle(::Rect(Rect.left + 1, Rect.top + 1, Rect.right - 1,
    // Rect.Bottom - 1));
  }
  ...
  if (ARow == 0 || ACol == 0) {
  ...
  } else if ((SG->Cells[ACol][0] == MLText.Fold ||
    ...
    Canvas->Rectangle(Rect.left + 2, Rect.top + 2, Rect.left + 13,
                      Rect.bottom - 2);
    Canvas->Rectangle(Rect.right - 13, Rect.top + 2, Rect.right - 2,
                      Rect.bottom - 2);
    Canvas->Brush->Style = bsClear;
    Canvas->Font->Color = clBlack;
    Canvas->TextOut(Rect.left + 2 + 3, Rect.top + 2, "-");
    Canvas->TextOut(Rect.right - 13 + 3, Rect.top + 2, "+");

    // Size
    UnicodeString S = IntToStr(
        (int)(log(m_Document->GetLabelByIndex(type, ARow - 2)->m_nSize /
                  100.0) *
                  (8 / log(4)) +
              100.5) -
        100);
    Canvas->TextOut((Rect.left + Rect.right - Canvas->TextWidth(S)) / 2,
                    Rect.top + 2, S);
  }
}
// ---------------------------------------------------------------------------
...
void __fastcall TFo_View::TB_SizeLimitationChange(TObject *Sender) {
  SettingView.m_nSizeLimitation =
      (int)(100 * exp(TB_SizeLimitation->Position / (8 / log(4))) + 100) - 100;
