...
void TFo_Search::ApplyLanguageSetting() {
  ...
  if (bReplace) {
    UnicodeString Replace = "&Replace";
    Replace = Ini->ReadString("Search", "Replace", Replace);
    Caption = DeleteActionKey(Replace);
    Bu_FindNext->Caption = Replace;

    UnicodeString ReplaceAll = "Replace All";
    ReplaceAll = Ini->ReadString("Search", "ReplaceAll", ReplaceAll);
    Ch_GlobalSearch->Caption = ReplaceAll;

    La_ReplaceTo->Caption =
        Ini->ReadString("Search", "La_ReplaceTo", La_ReplaceTo->Caption);
  } else {
    Caption = Ini->ReadString("Search", "Caption", Caption);
    Ch_GlobalSearch->Caption =
        Ini->ReadString("Search", "Ch_GlobalSearch", Ch_GlobalSearch->Caption);
    Bu_FindNext->Caption =
        Ini->ReadString("Search", "Bu_FindNext", Bu_FindNext->Caption);
  }
  La_Keyword->Caption =
      Ini->ReadString("Search", "La_Keyword", La_Keyword->Caption);
  Ch_CardTitle->Caption =
      Ini->ReadString("Search", "Ch_CardTitle", Ch_CardTitle->Caption);
  Ch_Text->Caption = Ini->ReadString("Search", "Ch_Text", Ch_Text->Caption);
  Ch_MatchCase->Caption =
      Ini->ReadString("Search", "Ch_MatchCase", Ch_MatchCase->Caption);
  Ch_InvisibleCard->Caption =
      Ini->ReadString("Search", "Ch_InvisibleCard", Ch_InvisibleCard->Caption);
  Ch_Backward->Caption =
      Ini->ReadString("Search", "Ch_Backward", Ch_Backward->Caption);

  Bu_Cancel->Caption =
      Ini->ReadString("Dialog", "Bu_Cancel", Bu_Cancel->Caption);

  delete Ini;
}

// ---------------------------------------------------------------------------
__fastcall TFo_Search::TFo_Search(TComponent *Owner) : TForm(Owner) {}

// ---------------------------------------------------------------------------
void __fastcall TFo_Search::Bu_CancelClick(TObject *Sender) { Close(); }
...
void __fastcall TFo_Search::PM_CopyClick(TObject *Sender) {
  ...
  Co_Keyword->SelStart = Ed_Keyword->SelStart;
  Co_Keyword->SelLength = Ed_Keyword->SelLength;
  // Clipboard()->SetTextBuf(Co_Keyword->Text.SubString(Co_Keyword->SelStart +
  // 1, Co_Keyword->SelLength).c_str());
}
// ---------------------------------------------------------------------------
