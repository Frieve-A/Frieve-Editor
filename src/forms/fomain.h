...
public:
    ...
    void LoadSub(UnicodeString FN, bool bSoftLoad = false, bool bRefreshRecent = true);
...
private:
    ...
    void TextEditBackupSub(UnicodeString Action, int CardID = -1, int SelStart = 0, int SelLength = 0);//テキスト編集前にUndo用バックアップを行う

    //保存用
    void ExportBMP(int operation, UnicodeString Text);//ブラウザ画像を保存orコピー(0=BMP, 1=JPEG, 2=コピー）
	void ExportCardText(int operation, bool btitle, UnicodeString Text);//カード本文を保存orコピー
	void ExportHierarchicalText(TStringList *SL, int CurrentLevel, UnicodeString HChar, TCard *CurrentParent);//階層テキスト出力のためのサブルーチン。CurrentParentにぶら下がっているすべてのカードについて、その下の階層を出力させる。このルーチンは再帰的に呼ばれる
private:
    //描画用
