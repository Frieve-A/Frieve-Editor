...
typedef struct MovingVHLineData_ {
  ...
  int nType;    // 0=top to bottom, 1=bottom to top, 2=left to right, 3=right to
                // left
} MovingVHLineData;
...
void TFo_Main::BGAnimation_MovingVHLine(TCanvas *C) {
  ...
  if (emptyindex >= 0 && rand() % 2) {
    MovingVHLineData *Data =
        (MovingVHLineData
             *)&m_BGAnimationBuf[emptyindex * BGANIMATIONOBJECTSIZE];
    Data->bExist = true;
    Data->fSpeed = (rand() % 9000 + 1000) * 0.0001f * 0.05f;
    Data->fWidth = rand() % 3000 * 0.001f + 1; // 1-3 pixels
    Data->Color = HalfColor(HalfColor((rand() % 256) | ((rand() % 256) << 8) |
                                          ((rand() % 256) << 16),
                                      m_nFGColor, 0.33f),
                            m_nBGColor, 0.875f);
...
void TFo_Main::BGAnimation_Bubble(TCanvas *C) {
  ...
  for (int i = 0; i < 256 && i < BGANIMATIONMAXOBJECTS; i++) {
    ...
    if (Data->bExist) {
      ...
      Data->fX +=
          (rand() % 201 - 100) * 0.003 * Data->fSpeed * m_fBGAnimationSpeed;
      // Data->Color = HalfColor(Data->Color, HalfColor(HalfColor((rand() % 256)
      // | ((rand() % 256) << 8) | ((rand() % 256) << 16),m_nFGColor, 0.33f),
      // m_nBGColor, 0.875f), 0.1f);
      P->Color = Data->Color;
  ...
  if (emptyindex >= 0 && rand()) {
    ...
    Data->Color = HalfColor(HalfColor((rand() % 128) | ((rand() % 256) << 8) |
                                          ((rand() % 256) << 16),
                                      m_nFGColor, 0.33f),
                            m_nBGColor, 0.5f);
...
void TFo_Main::BGAnimation_Snow(TCanvas *C) {
  ...
  for (int i = 0; i < 256 && i < BGANIMATIONMAXOBJECTS; i++) {
    ...
    if (Data->bExist) {
      ...
      Data->aX += (rand() % 201 - 100) * 0.003 * Data->fSpeed * 0.13f *
                  m_fBGAnimationSpeed;
      Data->aX *= 0.9f;
  ...
  if (emptyindex >= 0 && rand()) {
    ...
    Data->Color =
        HalfColor(HalfColor(m_nBGColor, 0x808080, 0.5f), 0xffffff, 0.4f);
...
void TFo_Main::BGAnimation_CherryBlossom(TCanvas *C) {
  ...
  for (int i = 0; i < 256 && i < BGANIMATIONMAXOBJECTS; i++) {
    ...
    if (Data->bExist) {
      ...
      Data->fAY =
          Data->fAY * 0.85f + fabs(sin(Data->fT1) * cos(Data->fT2)) + 0.1f;
      ...
      if (Data->fX < -0.1f || Data->fX > 1.1f || Data->fY > 1.1f) {
        ...
        for (int ip = 0; ip < 6; ip++) {
          ...
          float fx = (CBShape[ip * 2] * cos(Data->fT1) -
                      CBShape[ip * 2 + 1] * sin(Data->fT1)) *
                         ct3 +
                     (CBShape[ip * 2] * sin(Data->fT1) -
                      CBShape[ip * 2 + 1] * cos(Data->fT1)) *
                         st2 * st3;
          float fy = (CBShape[ip * 2] * sin(Data->fT1) -
                      CBShape[ip * 2 + 1] * cos(Data->fT1)) *
                     ct2;
  ...
  if (emptyindex >= 0 && rand()) {
    ...
    Data->fSpeed = (rand() % 3000 + 7000) * 0.0001f * 0.1f * (Data->fSize + 1);
    Data->Color =
        HalfColor(TColor(0xc890ff), TColor(0xe8d0ff), (rand() % 101) * 0.01f);
    Data->fX = (rand() % 10001) * 0.0001f;
    Data->fY = 0.0f;
    Data->fAX = 0.0f;
    Data->fAY = 1.0f;
