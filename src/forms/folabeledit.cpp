...
void TFo_LabelEdit::ApplyLanguageSetting() {
	...
	Bu_Delete->Caption = Ini->ReadString("LabelEdit", "Bu_Delete",
		Bu_Delete->Caption);
...
void __fastcall TFo_LabelEdit::SG_LabelsDrawCell(TObject *Sender, int ACol,
	...
	if (ARow > 0 && ACol == 1) {
		// Color
		Canvas->Brush->Color = (TColor)m_Document->GetLabelByIndex(m_LType,
			ARow - 1)->m_nColor;
		Canvas->FillRect(::Rect(Rect.Left + 2, Rect.Top + 2, Rect.Right - 2,
			Rect.Bottom - 2));
	}
...
void __fastcall TFo_LabelEdit::SG_LabelsDblClick(TObject *Sender) {
	...
	if (ACol == 0) {
		...
		if (CD->Execute()) {
			BackupSub();
			m_Document->GetLabelByIndex(m_LType, ARow - 1)->m_nColor =
				(int)CD->Color;
			m_Document->RefreshLabel();
		}
	}
...
void __fastcall TFo_LabelEdit::FormClose(TObject *Sender, TCloseAction &Action)
{
