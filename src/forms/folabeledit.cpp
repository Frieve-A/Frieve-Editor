...
void TFo_LabelEdit::ApplyLanguageSetting() {
  ...
  Bu_Delete->Caption =
      Ini->ReadString("LabelEdit", "Bu_Delete", Bu_Delete->Caption);
...
__fastcall TFo_LabelEdit::TFo_LabelEdit(TComponent *Owner)
    : TForm(Owner), m_Document(NULL), m_UndoRedo(NULL), m_bEdited(false),
      m_LType(0), m_nLastCol(0), m_nLastRow(0) {}
...
void __fastcall TFo_LabelEdit::SG_LabelsDrawCell(TObject *Sender, int ACol,
  ...
  if (ARow > 0 && ACol == 1) {
    // Color
    Canvas->Brush->Color =
        (TColor)m_Document->GetLabelByIndex(m_LType, ARow - 1)->m_nColor;
    Canvas->FillRect(
...
void __fastcall TFo_LabelEdit::SG_LabelsDblClick(TObject *Sender) {
  ...
  if (ACol == 0) {
    ...
    Fo_EditText->Ed_Text->Text =
        m_Document->GetLabelByIndex(m_LType, ARow - 1)->m_Name;
