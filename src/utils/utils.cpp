...
TFastIniSection::~TFastIniSection() { delete m_KeyValues; }
...
UnicodeString TFastIniSection::Values(int index) {
  ...
  return m_KeyValues->Strings[index].SubString(
      p + 1, m_KeyValues->Strings[index].Length());
}
...
void TFastIni::LoadFromString(TStringList *SL) {
  ...
  if (lastline < SL->Count) {
    TFastIniSection *FIS =
        new TFastIniSection(SectionName, SL, lastline, SL->Count - 1);
    m_Sections->Add(FIS);
  }
}
...
int TFastIni::ReadInteger(UnicodeString Section, UnicodeString Key,
                          int Default) {
  TFastIniSection *FIS = SearchSection(Section);
  if (FIS) {
...
// ---------------------------------------------------------------------------
int CountStr(WideString S,
             WideString CountChar) // Count occurrences of CountChar
{
  int result = 0;
  while (true) {
