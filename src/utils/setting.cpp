...
void TSettingFile::WriteToIni(TIniFile *Ini, UnicodeString Section)
{
    ...
    for (int i = 0 ; i < 10 ; i++){
        Ini->WriteString(Section, UnicodeString("HistoryF") + i, m_RecentFolders[i]);
    }
...
void TSettingFile::ReadFromIni(TIniFile *Ini, UnicodeString Section)
{
    ...
    for (int i = 0 ; i < 10 ; i++){
        m_RecentFiles[i] = Ini->ReadString(Section, UnicodeString("History") + i, "");
    }
    for (int i = 0 ; i < 10 ; i++){
		m_RecentFolders[i] = Ini->ReadString(Section, UnicodeString("HistoryF") + i, "");
    }
    ...
    if (m_PlugIn != ""){
        if (FileExists(m_PlugIn)){
            ...
            fepOperation = (bool __stdcall (*)(IDocCtrl *, wchar_t *))GetProcAddress(m_hPlugIn, "fepOperation");
