...
// ---------------------------------------------------------------------------
int &TDMatrix::ID(int idx) { return m_IDs[idx]; }
...
// ---------------------------------------------------------------------------
// Destructor
__fastcall TFList::~TFList() { Clear(); }
...
// ---------------------------------------------------------------------------
// Get count
int TFList::FCount() { return m_nCount; }
...
// ---------------------------------------------------------------------------
// Access
void *TFList::FItems(int nindex) { return m_Data[nindex]; }

void TFList::FWrite(int nindex, void *P) { m_Data[nindex] = P; }
...
TTextDecomposer::TTextDecomposer(TWideStringList *SL, int maxcombi,
                                 int maxcount, float pgmin, float pgmax,
                                 float &pgpos, bool &terminated, int option)
  ...
  for (int i = 0; i < m_nMaxCombi && !terminated; i++) {
        ...
        if (i > 0) {
          ...
          for (int i3 = 0; i3 < tmp2count; i3++) {
            ...
            if (add) {                               // Add Tmp2->Strings(i3)
              ...
              // WideString TSC = ((WideString)TS1).SubString(1, i);// 1-char
              // shorter target
              int cmp = 1;
          ...
          for (int i3 = 0; i3 < tmp2count; i3++) {
            ...
            bool add =
                i3 == 0 ? true : *TS1 != Ret && WideCompareStr(*WSL, *TS1) != 0;
            if (add) {
              // Tmp->AddA(*TS1);
              Tmp->AddP(TS1);
              Tmp2->SetNULL(i3);
    ...
    if (i > 0) {
      ...
      for (int i3 = 0; i3 < count; i3++) {
        WideString TSC =
            gram->Strings(i3).SubString(1, i); // 1-char shorter text
        int cmp = 1;
        while (lastindex < lastcount) {
    ...
    if (i > 0) {
      ...
      for (int i3 = 0; i3 < count; i3++) {
        WideString TSC =
            gram->Strings(i3).SubString(2, i); // 1-char shorter text
        int cmp = 1;
        while (lastindex < lastcount) {
          if ((cmp = WideCompareStr(lastgram->Strings(lastindex), TSC)) < 0) {
            lastindex++;
...
// TWideStringList
// ---------------------------------------------------------------------------
_fastcall TWideStringList::~TWideStringList() { Clear(); }
...
// ---------------------------------------------------------------------------
void TWideStringList::AddP(WideString *S) { TFListforTP::Add(S); }

// ---------------------------------------------------------------------------
void TWideStringList::SetNULL(int nindex) { Items[nindex] = NULL; }
...
// ---------------------------------------------------------------------------
// Ascending sort
void TWideStringList::Sort() { TFListforTP::Sort(WideStringSortCompare); }
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// TWSandValueList
// ---------------------------------------------------------------------------
_fastcall TWSandValueList::~TWSandValueList() { Clear(); }
...
// ---------------------------------------------------------------------------
void TSMatrix::Dummy() { Add(0, 0, 0.0f); }
...
// ---------------------------------------------------------------------------
void TSMatrix::Finalize(bool bAdd, float *pgpos, bool *terminated,
                        float pgstep) {
  bool term = false;
  if (terminated == NULL) {
    terminated = &term;
      ...
      for (int ib = 0; ib <= wcount / block; ib++) {
        ...
        for (int i = 0; i < SimList->Count; i++) {
          ...
          for (int ix = i + 1; ix < SimList->Count;) {
            ...
            if ((CCV->Col1 == CCV2->Col1) || (CCV->Col2 == CCV2->Col1) ||
                (CCV->Col2 == CCV2->Col2) || (CCV->Col2 == CCV2->Col2)) {
              // Remove duplicate
