...
//TUndoRedo系
//---------------------------------------------------------------------------
TUndoRedoData::TUndoRedoData(TDocument *Doc, UnicodeString Name, int CardID, int SelStart, int SelLength) :
...
//---------------------------------------------------------------------------
void TUndoRedo::Backup(TDocument *Doc, UnicodeString editname, int CardID, int SelStart, int SelLength)
{
	ClearRedos();
...
bool TDocument::SaveToString(TStringList *SL)
{
	...
	SL->Add(UnicodeString("ArrangeMode=") + IntToStr(nReqArrangeMode));//0から順にRepulsion、Link、Label、Index
	SL->Add(UnicodeString("AutoScroll=") + IntToStr(bReqAutoScroll));//オートスクロール
	...
	SL->Add(UnicodeString("DateLimitationDateType=") + IntToStr(ReqDateLimitationDateType));
	SL->Add(UnicodeString("DateLimitationType=") + IntToStr(ReqDateLimitationType));
...
bool TDocument::Load_Old(UnicodeString FN)
{
    ...
    //各カード保存用フォルダ
    UnicodeString Dir = m_FN.SubString(1, m_FN.Length() - ExtractFileExt(m_FN).Length());
