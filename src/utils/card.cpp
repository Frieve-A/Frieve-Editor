...
TCard::TCard(TCard &Card) {
  ...
  m_nCreatedOrder =
      Card.m_nCreatedOrder; // Date normalized to 0.0~100.0 by sort order
  m_nUpdatedOrder =
      Card.m_nUpdatedOrder; // Date normalized to 0.0~100.0 by sort order
  m_nViewedOrder =
      Card.m_nViewedOrder;    // Date normalized to 0.0~100.0 by sort order
  ...
  m_fMatrixSpeed =
      Card.m_fMatrixSpeed; // Movement speed between grid cells (0~1.0)

  m_bLabelIsFold =
      Card.m_bLabelIsFold; // Whether all labels on this card are folded
