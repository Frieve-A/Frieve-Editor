...
void TLink::Decode_006(UnicodeString S) {
  ...
  m_nShape =
      StrToIntDef(S.SubString(1, p - 1), SettingView.m_nLinkDefaultShape);
  S = S.SubString(p + 1, S.Length());
...
UnicodeString TLink::Encode() {
  UnicodeString S =
      ...
      UnicodeString(",") + UnicodeString("Di") + IntToStr(m_bDirection) + // Di
