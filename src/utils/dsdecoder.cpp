...
const GUID MEDIATYPE_Video = {0x73646976,
                              0x0000,
                              0x0010,
                              {0x80, 0x00, 0x00, 0xAA, 0x00, 0x38, 0x9B, 0x71}};
const GUID MEDIATYPE_Audio = {0x73647561,
                              0x0000,
                              0x0010,
                              {0x80, 0x00, 0x00, 0xAA, 0x00, 0x38, 0x9B, 0x71}};
...
const GUID TIME_FORMAT_MEDIA_TIME = {
    ...
    0x8C82,
    0x11CF,
    {0xBC, 0x0C, 0x00, 0xAA, 0x00, 0xAC, 0x74, 0xF6}};

HRESULT __stdcall TDSDecoder::SampleCB(double SampleTime,
...
__fastcall TDSDecoder::TDSDecoder(bool CreateSuspended)
    : TThread(CreateSuspended), CBBMP(NULL), CBWAVE(NULL), m_bGrub(false),
      m_bUnsync(false), m_bLoop(false), m_fLoopStartPos(0.0),
      m_fLoopEndPos(-1.0),

      m_bExecuted(false),
      m_bPrepared(false), // Whether playback is prepared (format obtained)
      m_bWaitToPlay(
          false), // Wait before starting. Playback starts when set to false
...
void __fastcall TDSDecoder::Execute() {
  ...
  if (pGraphBuilder) {
    ...
    pGraphBuilder->QueryInterface(IID_IMediaFilter, (LPVOID *)&pMediaFilter);
  ...
  while (!Terminated) {
    ...
    if (m_bLoop) {
      if (len > 0.0 && ((len < pos + 1) || (m_fLoopEndPos > m_fLoopStartPos &&
                                            m_fLoopEndPos < pos + 1))) {
        if (m_fLoopStartPos > 0.0) {
          SetPosition(m_fLoopStartPos);
        } else {
