...
public:
  ...
  virtual int
  SearchBrother(int CurrentID, int ParentID, bool bInverse, bool bChild,
                bool bFocus) = 0; // Find sibling nodes clockwise (or counter)
  virtual int SearchLast(int CardID,
                         bool bFocus) = 0; // Find previously displayed card
  virtual void RefreshCardLevel() = 0; // Set hierarchy level from Card->m_bTop
                                       // and links, sort cards
...
public:
  ...
  int SearchParent(int CardID, bool bChild,
                   bool bFocus); // Search parent (or child) card
  ...
  void RefreshCardLevel(); // Set hierarchy level from Card->m_bTop and links,
                           // sort cards
...
public:
  ...
  bool LabelIsSame(TCard *Card1,
                   TCard *Card2); // Check if two cards have same labels
