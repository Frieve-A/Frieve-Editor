...
public:
    //ファイル
    virtual bool Load(UnicodeString FN, bool bSoftLoad = false) = 0;//SoftLoadでは、元データの座標を維持
    virtual bool Save() = 0;
...
public:
    //ファイル
    bool Load(UnicodeString FN, bool bSoftLoad = false);//SoftLoadでは、元データの座標を維持
    bool Save();
...
public:
    ...
    int m_nSelLength;
public:
    TUndoRedoData(TDocument *Doc, UnicodeString Name, int m_nCardID = -1, int SelStart = 0, int SelLength = 0);
...
public:
    ...
    void ClearUndos();
    void ClearRedos();
	void Backup(TDocument *Doc, UnicodeString editname, int CardID = -1, int SelStart = 0, int SelLength = 0);
