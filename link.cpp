...
//---------------------------------------------------------------------------
void TLink::Decode(AnsiString S)
{
    WideString S2 = S;
...
//---------------------------------------------------------------------------
void TLink::Decode_006(AnsiString S)
{
    int p;
    p = S.Pos(",");
    ...
    m_nShape = StrToIntDef(S.SubString(1, p - 1), SettingView.m_nLinkDefaultShape);
    S = S.SubString(p + 1, S.Length());
...
//---------------------------------------------------------------------------
void TLink::Decode_005(AnsiString S)
{
    int p;
    p = S.Pos(",");
    m_nFromID = StrToIntDef(S.SubString(1, p - 1), 0);
...
//---------------------------------------------------------------------------
AnsiString TLink::Encode()
{
    AnsiString S =
        AnsiString("Fr") + IntToStr(m_nFromID) +//Fr
        AnsiString(",") +
        AnsiString("De") + IntToStr(m_nDestID) +//De
        AnsiString(",") +
        AnsiString("Di") + IntToStr(m_bDirection) +//Di
        AnsiString(",") +
        AnsiString("Sh") + IntToStr(m_nShape);//Sh
        /*
        AnsiString(",") +
        AnsiString("") + IntToStr(m_Labels->Count);
        */

    for (int i = 0 ; i < m_Labels->Count ; i++){
        S +=
            AnsiString(",La") +
            IntToStr(m_Labels->GetLabel(i));//La
    }
