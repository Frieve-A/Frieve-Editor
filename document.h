...
public:
    ...
    virtual void SetCardTitle(TCard *Card, char *szChar) = 0;
    virtual void SetCardText(TCard *Card, char *szChar) = 0;
    //virtual void SetCardText(int nID, char *szChar) = 0;
    ...
    virtual int SearchParent(int CardID, bool bChild, bool bFocus) = 0;//親（もしくは子）カードを捜す
    virtual int SearchBrother(int CurrentID, int ParentID, bool bInverse, bool bChild, bool bFocus) = 0;//時計回り（もしくは逆周り）に兄弟ノードを探す
...
public:
    ...
    virtual void SetLinkName(TLink *Link, char *szChar) = 0;
...
public:
    ...
    virtual TCardLabel *GetLabel(int ltype, AnsiString S) = 0;
    virtual void SetLabelName(TCardLabel *Label, char *szChar) = 0;
    ...
    virtual bool LabelIsSame(TCard *Card1, TCard *Card2) = 0;//2つのカードのラベルが同じか調べる
public:
    //表示更新用
...
public:
    //ファイル
    virtual bool Load(AnsiString FN, bool bSoftLoad = false) = 0;//SoftLoadでは、元データの座標を維持
    virtual bool Save() = 0;
    virtual bool LoadFromString(TStringList *SL, AnsiString FN) = 0;
    virtual bool SoftLoadFromString(TStringList *SL, AnsiString FN) = 0;
...
public:
    ...
    void SetCardTitle(TCard *Card, char *szChar);
    void SetCardText(TCard *Card, char *szChar);
    ...
    int SearchBrother(int CurrentID, int ParentID, bool bInverse, bool bChild, bool bFocus);//時計回り（もしくは逆周り）に兄弟ノードを探す
    ...
    TList *GetRelatedCard(bool bInverse, bool bVisibleOnly);//各カードから張られたリンク先のカードをIndex順に入れる
...
public:
    ...
    void SetLinkName(TLink *Link, char *szChar);
...
public:
    ...
    TCardLabel *GetLabel(int ltype, AnsiString S);
    void SetLabelName(TCardLabel *Label, char *szChar);
...
public:
    //ファイル
    bool Load(AnsiString FN, bool bSoftLoad = false);//SoftLoadでは、元データの座標を維持
    bool Save();
    bool LoadFromString(TStringList *SL, AnsiString FN);
    bool SoftLoadFromString(TStringList *SL, AnsiString FN);
    bool SaveToString(TStringList *SL);

    //フォルダ分けしていた時の互換用
    bool Load_Old(AnsiString FN);
    bool Save_Old();
public:
...
public:
    ...
    int m_nSelLength;
public:
    TUndoRedoData(TDocument *Doc, AnsiString Name, int m_nCardID = -1, int SelStart = 0, int SelLength = 0);
...
public:
    ...
    void ClearUndos();
    void ClearRedos();
    void Backup(TDocument *Doc, char *editname, int CardID = -1, int SelStart = 0, int SelLength = 0);
...
extern int bReqSizeLimitation, bReqLinkLimitation, bReqDateLimitation;//表示制限のON/OFF
extern int nReqSizeLimitation;
extern int nReqLinkLimitation, bReqLinkDirection, bReqLinkBackward, nReqLinkTarget;
