...
const GUID MEDIASUBTYPE_WAVE = {0xE436EB8B, 0x524F, 0x11CE, {0x9F, 0x53, 0x00, 0x20, 0xAF, 0x0B, 0xA7, 0x70}};
...
const GUID TIME_FORMAT_MEDIA_TIME = {0x7B785574, 0x8C82, 0x11CF, {0xBC, 0x0C, 0x00, 0xAA, 0x00, 0xAC, 0x74, 0xF6}};

HRESULT __stdcall TDSDecoder::SampleCB(
    double SampleTime,
...
HRESULT __stdcall TDSDecoder::BufferCB(
    double SampleTime,
    BYTE *pBuffer,
    long nBufferSize)
{
    if (CBWAVE){
        ...
        pSampleGrabber->GetCurrentBuffer(&nBufferSize,
            (long*)pBuffer);
        void *DIBPtr = NULL;
        void *BitmapHandle = CreateDIBSection(0, (BITMAPINFO*)&pVideoInfoHeader->bmiHeader,
...
void TDSDecoder::SetPosition(double ms)
{
    ...
    */
    CurrentPos = (LONGLONG)(ms * 10000);
    if (m_bPrepared && pMediaSeeking){
        pMediaSeeking->SetPositions(&CurrentPos, AM_SEEKING_AbsolutePositioning, &EndPos, AM_SEEKING_NoPositioning);
    }
}
...
void __fastcall TDSDecoder::Execute()
{
    ...
    if (pGraphBuilder){
        pGraphBuilder->AddSourceFilter(WideString(m_FN), L"SourceFilter", &pSourceFilter);
    }
    ...
    while (!Terminated){
        ...
        if (m_bLoop){
            if (len > 0.0 &&
                ((len < pos + 1) || (m_fLoopEndPos > m_fLoopStartPos && m_fLoopEndPos < pos + 1))){
                if (m_fLoopStartPos > 0.0){
                    SetPosition(m_fLoopStartPos);
                }else{
                    SetPosition(0.0);
    ...
    if (pFilter_NULLRenderer){ pFilter_NULLRenderer->Release(); }
