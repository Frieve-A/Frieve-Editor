...
//---------------------------------------------------------------------------
TCardImage::TCardImage(AnsiString FN) :
    m_FN(FN),
    m_bExist(false),
    m_bUsed(true),
    m_Jpg(NULL),
...
//---------------------------------------------------------------------------
TCardVideo::TCardVideo(AnsiString FN) :
    m_FN(FN),
    m_bExist(false),
    m_bUsed(true),
    m_Video(NULL)
        ...
        if (fcount > 1 || LowerCase(ExtractFileExt(FN)) != ".fip"){
            ...
            if (m_nTargetCard >= 0 && m_nTargetCard == m_nCurrentCard){
                ...
                for (int i = 0 ; i < fcount ; i++){
                    ...
                    Card->m_Title = Card->m_Title.SubString(1, Card->m_Title.Length() - ExtractFileExt(Card->m_Title).Length());
...
//---------------------------------------------------------------------------
void TFo_Main::BackupSub(AnsiString Action)
{
    if (!m_bDoNotBackup){
...
//---------------------------------------------------------------------------
void TFo_Main::RefreshRecent(AnsiString FN)
{
    //履歴更新
    if (FN != "" &&
        FN != (ExtractFilePath(ParamStr(0)) + AnsiString("help.fip"))){
        //ファイル名
        for (int i = 0 ; i < 10 ; i++){
        ...
        for (int i = 0 ; i < 10 ; i++){
            if (SettingFile.m_RecentFolders[i] == FN){
                ...
                if (i > 0){
                    for (int i2 = i ; i2 >= 1 ; i2--){
                        SettingFile.m_RecentFolders[i2] = SettingFile.m_RecentFolders[i2 - 1];
                    }
                }
    ...
    for (int i = 0 ; i < 10 ; i++){
        if (SettingFile.m_RecentFiles[i] != ""){
            if (i < 9){
                MI_RecentFiles[i]->Caption = AnsiString("&") + IntToStr(i + 1) + " " + SettingFile.m_RecentFiles[i];
            }else{
                MI_RecentFiles[i]->Caption = AnsiString("1&0 ") + SettingFile.m_RecentFiles[i];
            }
        ...
        if (SettingFile.m_RecentFolders[i] != ""){
            if (i < 9){
                MI_RecentFolders[i]->Caption = AnsiString("&") + IntToStr(i + 1) + " " + SettingFile.m_RecentFolders[i] + "...";
            }else{
                MI_RecentFolders[i]->Caption = AnsiString("1&0 ") + SettingFile.m_RecentFolders[i] + "...";
            }
...
void __fastcall TFo_Main::FormCreate(TObject *Sender)
{
    ...
    */
    ...
    //初期設定を高速読み込み
    TFastIni *FIni = new TFastIni(ExtractFilePath(ParamStr(0)) + "setting.ini");
    SettingFile.ReadFromIni(FIni, "File");
    SettingView.ReadFromIni(FIni, "View");
    delete FIni;

    FIni = new TFastIni(ExtractFilePath(ParamStr(0)) + "setting2.ini");
    Setting2Function.ReadFromIni(FIni);
    delete FIni;

    //実行中の読み書きは通常のIniFileで
    ...
    */
    for (int i = 0 ; i < Setting2Function.m_WebSearch->Count * 4 ; i++){
        ...
        case 0:MI = new TMenuItem(ME_WebSearch);break;
        case 1:MI = new TMenuItem(PE_Sep1->Owner);break;
        case 2:MI = new TMenuItem(PBC_WebSearch);break;
        case 3:MI = new TMenuItem(PBL_WebSearch);break;
        }
        MI->Caption = Setting2Function.m_WebSearch->Names[i % Setting2Function.m_WebSearch->Count];
        ...
        case 0:
            ...
            ME_WebSearch->Add(MI);
            break;
        case 1:PM_Editor->Items->Insert(inspos++, MI);break;
        case 2:PBC_WebSearch->Add(MI);break;
        case 3:PBL_WebSearch->Add(MI);break;
        }
    }
...
void TFo_Main::RefreshLabel()
{
    ...
    if (m_nTargetCard < 0){
        ...
        if (Card) for (int i = 0 ; i < MAXLABELS ; i++){
            if (i < Card->m_Labels->Count){
                //ラベルあり
                Bu_Label[i]->Caption = m_Document->GetLabelByIndex(0, Card->m_Labels->GetLabel(i) - 1)->m_Name;
    ...
    if (m_nTargetLink < 0){
        ...
        if (Link) for (int i = 0 ; i < MAXLABELS ; i++){
            if (i < Link->m_Labels->Count){
                //ラベルあり
                Bu_LinkLabel[i]->Caption = m_Document->GetLabelByIndex(1, Link->m_Labels->GetLabel(i) - 1)->m_Name;
...
{
    return Bu_ArrangeType->Tag >= 1000 && Bu_ArrangeType->Tag < 1999 && SB_Arrange->Down;
}
//---------------------------------------------------------------------------
void __fastcall TFo_Main::Ti_CheckTimer(TObject *Sender)
{
    ...
    //Caption
    {
        ...
        if (SettingView.m_nSpecialPaint){
            ...
            S = AnsiString(AppTitle) + " " + m_Document->m_FN;
    ...
    //ファイルリスト
    {
        ...
        if (pos.x >= Left && pos.x < Left + Width && pos.y >= Top && pos.y < Top + Height){
            ...
            */
            if ((pos.x < Left + 16 && pos.y >= listpos.y && pos.y < listpos.y + Pa_List->Height) && !Pa_Files->Visible && m_nAnimation == 0 && Application->Active && !m_bMDownBrowser && SettingView.m_bFileList){
                CloseEditBox();
                ...
                */
                ...
                PB_Browser->Tag = 0;
                //*/
            }else if ((pos.x >= Left + Pa_List->Left + 32 || pos.y < listpos.y || pos.y > listpos.y + Pa_Files->Height || !Application->Active) && Pa_Files->Visible){
    ...
    bool RelatedTextRefreshRequest = false;//関連テキスト更新要求
    {
        ...
        if (SettingView.m_bEditorRelatedVisible != Pa_EditorRelated->Visible){
            ...
            RelatedTextRefreshRequest = Pa_EditorRelated->Visible;//表示にした場合は更新要求
        }
    }
    ...
    SelectionChanged |= m_Document->SearchCardIndex(m_nTargetCard) != LB_List->ItemIndex;
    SelectionChanged |= m_nCurrentCard != m_nTargetCard;
    bool ListChanged =
        LB_List->Count != m_Document->m_Cards->Count ||
        m_nRefreshListCount != m_Document->m_nRefreshListCount;
    if (SelectionChanged || ListChanged || Fo_View->m_bRefreshRequest || m_UndoRedo->m_bChanged){
        ...
        if (m_nCurrentCard >= 0 && m_nCurrentCard == m_nTargetCard){
            ...
            if (Card){
                AnsiString Text = Card->m_Lines->Text.SubString(1, Card->m_Lines->Text.Length() - 2);
                if (Text != RE_Edit->Text && Card->m_Lines->Text != RE_Edit->Text){
                    RE_Edit->Tag = 1;
                    RE_Edit->Lines->BeginUpdate();
                    int line = RE_Edit->Perform(EM_GETFIRSTVISIBLELINE, 0, 0);
                    RE_Edit->Text = Text;
                    if (m_nNextCardID == m_nCurrentCard){
                        int line2 = RE_Edit->Perform(EM_GETFIRSTVISIBLELINE, 0, 0);
                        if (line2 > line){
                            while (line2-- > line){
                                RE_Edit->Perform(EM_SCROLL, SB_LINEUP, 0);
                            }
                        }else{
                            while (line2++ < line){
                                RE_Edit->Perform(EM_SCROLL, SB_LINEDOWN, 0);
                            }
                        }
                ...
                AnsiString TB = DecodeES(Card->m_Title, "\r\n");
    ...
    if (m_nCurrentCard != m_nTargetCard){
        ...
        m_nLastTarget = -1;
        if (LastCard && Card){
            if (LastCard->m_nSelected > 0 && !LastCard->m_bSelected && Card->m_nSelected < 100){
            ...
            if (Card){
                ...
                if (Ed_Title->Text != Card->m_Title){
                    ...
                    if (SettingView.m_bRead) if (m_AgentChar && m_TTS){
                        m_AgentChar->Stop();
                        m_AgentChar->Speak(Variant(WideString(AnsiString("\\Pit=200\\\\Spd=200\\") + Card->m_Title.c_str())));
                    }
                    */
                    Ed_Title->Tag = 0;
                }
    ...
    if (RelatedTextRefreshRequest && Pa_EditorRelated->Visible){
        ...
        if (m_nTargetCard >= 0){
            ...
            for (int il = 0 ; il < m_Document->m_Links->Count ; il++){
                ...
                }else if (Link->m_nDestID == Card->m_nID && Link->m_nFromID != Card->m_nID){
                    Card2 = m_Document->GetCard(Link->m_nFromID);
                }

                if (Card2) if (Card2->m_Lines->Count > 0){
                    if (Card2->m_bVisible){
                        Str->Add(new AnsiString(
                            AnsiString("Title : ") + DecodeES(Card2->m_Title, " ") + AnsiString("\r\n\r\n") +
                            Card2->m_Lines->Text + AnsiString("\r\n\r\n")
                            ));
                    }else{
                        Str->Add(new AnsiString(
                            AnsiString("(Title : ") + DecodeES(Card2->m_Title, " ") + AnsiString(")\r\n\r\n") +
                    ...
                    if (!Card2->m_bVisible){
                        order += m_Document->m_Cards->Count;//非表示カードのテキストは後ろへ
                    }
            ...
            while (Str->Count){
                ...
                NewStr->Add(*(AnsiString*)Str->Items[minindex]);
                delete (AnsiString*)Str->Items[minindex];
                Str->Delete(minindex);
    ...
    if (m_nTargetCard >= 0 && Card){
        //サイズ表示変更
        int size = (int)(log(Card->m_nSize / 100.0) * (8 / log(4)) + 100.5) - 100;//8で4倍
        if (size > 8){
            size = 8;
        }else if (size < -8){
            size = -8;
    ...
    //Bu_ArrangeTypeのキャプション
    {
        ...
        case 0:S = PAT_Normalize->Caption;break;
        case 1:S = PAT_Repulsion->Caption;break;
        case 2:S = PAT_Link->Caption;break;
        case 3:S = PAT_Label->Caption;break;
        case 4:S = PAT_Index->Caption;break;
        case 102:S = PAT_SoftLink->Caption;break;
        case 103:S = PAT_SoftLabel->Caption;break;
        case 104:S = PAT_SoftIndex->Caption;break;
        case 200:S = PAT_Matrix->Caption;break;
        //case 202:S = "Link(Matrix)->Caption;break;
        //case 203:S = "Label(Matrix)->Caption;break;
        case 204:S = PAT_MatrixIndex->Caption;break;

        //500～999＝Similarity
        case 500:S = PAT_Similarity->Caption;break;
        case 600:S = PAT_SimilaritySoft->Caption;break;

        //1000～1999＝Tree
        case 1000:S = PAT_TreeRadial->Caption;break;
        }
        S = DeleteActionKey(S);
    ...
    if (SB_ArrangeRefresh->Enabled != (SB_Arrange->Down && Bu_ArrangeType->Tag >= 500 && Bu_ArrangeType->Tag <= 999)){
        SB_ArrangeRefresh->Enabled = (SB_Arrange->Down && Bu_ArrangeType->Tag >= 500 && Bu_ArrangeType->Tag <= 999);
    }

    //ブラウザ座標
    m_fTickerSpeed = (tgt - m_nLastTimeOut) * (0.1f / 120);//120msで0.1文字？//TickerSpeed
    m_fBGAnimationSpeed = (tgt - m_nLastTimeOut) * 0.001f;
    if (PC_Client->ActivePage == TS_Browser && (Application->Active || SettingView.m_bRedrawInBackground)/* && !m_bMDownBrowser*/){
        bool b = true;
        if (SettingFile.fepOperation){
            if (SettingFile.fepOperation(m_Document, "Browser_RedrawByTimer")){
                b = false;
            }
        }
        if (b){
            if (SB_Arrange->Down/* && timeGetTime() >= m_uMDownBrowserLast + 500*/){
                switch (Bu_ArrangeType->Tag){
                case 0:
                    //Normalize
            ...
            drawrequest2 |= SettingView.m_bScore || SettingView.m_bTickerVisible || SettingView.m_bBGAnimation;

            //オートスクロールによる再描画
            if (!(m_bMDownBrowser > 0 && m_bMDownBrowser < 4) && m_bMDownBrowser != 5){//4はoverviewから表示位置指定中なのでAutoScroll
                if ((SB_AutoScroll->Down || SB_AutoZoom->Down || m_nScrollTargetX > -65536) && timeGetTime() >= m_uMDownBrowserLast + 500){
            ...
            if (SettingView.m_bFocusCursor){
                if (SettingView.m_bFocusCursorAnimation){
                    ...
                    drawrequest2 |= m_nFocusCursorPos < 100 || (m_nFocusCursorPos % 100 < 50) != last;
    ...
    if (PC_Client->ActivePage == TS_Statistics){
        ...
        if (m_fStatisticsPos < 1.0f){
            m_fStatisticsPos = 1.03f -
                (1.03f - m_fStatisticsPos) * pow(0.5, (tgt - m_nLastTimeOut) / 150.0);//150msで半分1.0に近づく
            if (m_fStatisticsPos > 1.0f){
    ...
    */
    ...
    if (bSearchRequest){
        ...
        if (SearchKeyword != WideString("")){
            ...
            while (Fo_Search->Co_Keyword->Items->Count >= 100){
                Fo_Search->Co_Keyword->Items->Delete(Fo_Search->Co_Keyword->Items->Count - 1);
            }
            ...
            if (srbak & 0x40){
                ...
                if (!(srbak & 0x100)) {
                    ...
                    La_GlobalSearchCount->Caption = IntToStr(m_GlobalSearchResult->Count) + AnsiString(" Hits");
    ...
    {
        char *UndoName;
        bool CanUndo = (m_UndoRedo->GetCanUndo(&UndoName) || Ed_FindCard->Focused()) && (m_nAnimation == 0);
        ...
        AnsiString S = ME_Undo->Caption;
        int p = S.Pos(" - ");
        if (p){
            S = S.SubString(1, p - 1);
        }
        if (UndoName){
            S += AnsiString(" - ") + AnsiString(UndoName);
        }
    ...
    //Redo
    {
        ...
        if (RedoName){
            S += AnsiString(" - ") + AnsiString(RedoName);
        }
    ...
    //Edit
    {
        ...
        bool editenabled =
            ...
            (!RE_Edit->Focused() && !Me_EditorRelated->Focused() && !Ed_Title->Focused() && !Ed_TitleB->Focused() && !Ed_LinkTitle->Focused() && !Ed_FindCard->Focused() && !TB_DZoom->Focused() && m_nTargetCard >= 0);
...

    {
        ...
        if (ME_WebSearch->Enabled != websearchenabled || LastSelText != SelText){
            ...
            for (int i = 0 ; i < MI_WebSearch->Count ; i++){
                ...
                if (websearchenabled){
                    MI->Caption =
                        Setting2Function.m_WebSearch->Names[i % Setting2Function.m_WebSearch->Count] +
                        AnsiString(" \"") +
                        SelText +
                        AnsiString("\"");
                }else{
    ...
    */
    ...
    }
    {
        ...
        case 1:MI = MVC_Ticker1Line; break;
        case 2:MI = MVC_Ticker2Lines; break;
        }
        if (MI){
            if (!MI->Checked){
                MI->Checked = true;
    ...
    }
    {
        ...
        case 32:MI = MVI_32; break;
        case 40:MI = MVI_40; break;
        case 64:MI = MVI_64; break;
        case 80:MI = MVI_80; break;
        case 120:MI = MVI_120; break;
        case 160:MI = MVI_160; break;
        case 240:MI = MVI_240; break;
        case 320:MI = MVI_320; break;
        }
        if (MI){
            if (!MI->Checked){
                MI->Checked = true;
    ...
    */
    ...
    if (LB_List->Color != (TColor)SettingView.m_nBackgroundColor){
        ...
        */
        ...
        bool focused = TB_Zoom->Focused(); TB_Zoom->Visible = false; TB_Zoom->Visible = true; if (focused){TB_Zoom->SetFocus();}//なぜかTB_Zoomの背景色が異なる問題対策
    }
...
void TFo_Main::SetTextEditPos()
{
    ...
    case 1:
        //Browser右
        if (SettingView.m_nEditorWidthInBrowser == 0 || SettingView.m_nEditorWidthInBrowser >= TS_Browser->Width){
        ...
        */
        Sp_BrowserRight->Left = (TS_Browser->Width - Sc_Y->Width - SettingView.m_nEditorWidthInBrowser) - Sp_BrowserRight->Width;
        Sp_BrowserRight->Visible = true;
        RE_Edit->Parent = TS_Browser;
        RE_Edit->Left = TS_Browser->Width - Sc_Y->Width - SettingView.m_nEditorWidthInBrowser;
        RE_Edit->Align = alRight;
        break;
    case 2:
        //Browser下
        if (SettingView.m_nEditorHeightInBrowser == 0 || SettingView.m_nEditorHeightInBrowser >= TS_Browser->Height){
        ...
        */
        Sp_BrowserBottom->Top = TS_Browser->Height - Pa_BrowserBottom->Height - SettingView.m_nEditorHeightInBrowser - Sp_BrowserBottom->Height;
        Sp_BrowserBottom->Visible = true;
        RE_Edit->Top = TS_Browser->Height - Pa_BrowserBottom->Height - SettingView.m_nEditorHeightInBrowser;
        RE_Edit->Parent = TS_Browser;
...
void TFo_Main::SetEdTitleBPos(){
    ...
    if (cardindex >= 0){
        ...
        for (int il = 0 ; il < Card->m_Labels->Count ; il++){
            TCardLabel *Label = m_Document->GetLabelByIndex(0, Card->m_Labels->GetLabel(il) - 1);
            if (Label->m_bEnable){
            ...
            if (hImc){
                ...
                */
                ...
                if (err != IMM_ERROR_NODATA && err != IMM_ERROR_GENERAL && err > 0){
                    //M_File->Caption = buf;
                    long DW = Ed_TitleB->Perform(EM_POSFROMCHAR, Ed_TitleB->SelStart, 0);
        ...
        if (SettingView.m_bTickerVisible && Card->m_Lines->Count > 0){
            for (int il = 0 ; il < SettingView.m_nTickerLines && il < Card->m_Lines->Count ; il++){
...
//---------------------------------------------------------------------------
void TFo_Main::TextEditBackupSub(AnsiString Action, int CardID, int SelStart, int SelLength)
{
    if (!m_bDoNotBackup){
        //テキスト編集前にUndo用バックアップを行う
        unsigned int tgt = timeGetTime();
        if (tgt > m_nLastModified + UNDOBACKUPSPAN){
            m_UndoRedo->Backup(m_Document, Action.c_str(), CardID, SelStart, SelLength);
        }
        m_nLastModified = tgt;
    }
}
    ...
    if (m_nCurrentCard >= 0){
        if (RE_Edit->Tag == 0){
            ...
            if (Card){
                ...
                TextEditBackupSub(MLText.EditCardText, m_nCurrentCard, RE_Edit->SelStart, len);

                m_Document->SetCardText(Card, RE_Edit->Text.c_str());
                m_Document->RefreshList();
...
void TFo_Main::RefreshLaStatus()
{
    if (m_nCurrentCard >= 0){
        ...
        if (Card){
            ...
            //文字数
            int line = RE_Edit->Perform(EM_LINEFROMCHAR, -1, 0);
            S += AnsiString("X : ") + IntToStr(RE_Edit->SelStart - RE_Edit->Perform(EM_LINEINDEX, line, 0) + 1) + AnsiString("  ");

            S +=
                MLText.Line + AnsiString(" : ") + IntToStr(line + 1) +
                AnsiString("/") + IntToStr(Card->m_Lines->Count) + AnsiString("  ");
            S +=
                MLText.Len + AnsiString(" : ") + IntToStr(WideString(RE_Edit->Text).SubString(1, RE_Edit->SelStart + 1).Length() - line * 2) +
                AnsiString("/") + IntToStr(WideString(Card->m_Lines->Text).Length() - Card->m_Lines->Count * 2) + AnsiString("  ");
            S +=
                MLText.Size + AnsiString(" : ") + /*SizeToStr(RE_Edit->SelStart + 1) +
                AnsiString("/") + */SizeToStr(Card->m_Lines->Text.Length()) + AnsiString("  ");

            SB_Status->Panels->Items[0]->Text = S;

            S = "";
            //Date
            S += MLText.Created + AnsiString(" : ") + DateTimeToStr(Card->m_fCreated) + AnsiString("  ");
            S += MLText.Edited + AnsiString(" : ") + DateTimeToStr(Card->m_fUpdated) + AnsiString("  ");
            S += MLText.Viewed + AnsiString(" : ") + DateTimeToStr(Card->m_fViewed) + AnsiString("  ");
...
void TFo_Main::RefreshLinks()
{
    ...
    if (Card){
        for (int i = 0 ; i < m_Document->m_Links->Count ; i++){
            ...
            if (Card2){
                AnsiString S = DecodeES(Card2->m_Title, " ");

                if (Card2->m_Labels->Count){
                    AnsiString S2 = AnsiString("[");
                    for (int i2 = 0 ; i2 < Card2->m_Labels->Count ; i2++){
                        if (i2 > 0){
                            S2 += ", ";
                        }
                        S2 += m_Document->GetLabelByIndex(0, Card2->m_Labels->GetLabel(i2) - 1)->m_Name;
                    }
                    S2 += AnsiString("] ");
...
bool TFo_Main::Save()
{
    ...
    if (!result) {
        ShowMessage(MLText.FailedToSave + m_Document->m_FN + AnsiString("."));
    }
    Ti_Check->Enabled = true;
...
//---------------------------------------------------------------------------
void TFo_Main::LoadSub(AnsiString FN, bool bSoftLoad, bool bRefreshRecent)
{
    ...
    if (result){
        ...
        m_bContinuousLoad = FN == ExtractFilePath(ParamStr(0)) + "_continuousload.fip";
        if (m_bContinuousLoad){
...
void __fastcall TFo_Main::ME_UndoClick(TObject *Sender)
{
    if (Ed_FindCard->Focused()){
        ...
        m_UndoRedo->Undo(m_Document, m_nCurrentCard, RE_Edit->SelStart, RE_Edit->SelLength, &m_nNextCardID, &m_nNextSelStart, &m_nNextSelLength);
        if (AnsiString(undostr) == MLText.ArrangeCards){
            SB_Arrange->Down = false;
        }
    }
...
void __fastcall TFo_Main::LB_LinkDblClick(TObject *Sender)
{
    if (LB_Link->ItemIndex >= 0){
        TLink *Link = m_Document->GetLinkByIndex((int)m_LinkIndexes->Items[LB_Link->ItemIndex]);
        if (Link){
            m_Document->ClearCardSelection();
            if (Link->m_nFromID == m_nCurrentCard){
...
void __fastcall TFo_Main::PI_DeleteLinkClick(TObject *Sender)
{
    if (LB_Link->ItemIndex >= 0){
        ...
        m_Document->DeleteLinkByIndex((int)m_LinkIndexes->Items[LB_Link->ItemIndex]);
    }
}
//---------------------------------------------------------------------------
void TFo_Main::CalcCardSize(TCanvas *C, TCard *Card, int Size, int cardindex)
{
    ...
    if (Drawing) {
        ...
        m_CardHeight[cardindex] += SettingView.m_nImageLimitation * 0.75 * imagex * 0.01;
    }
        ...
        if (Image->m_bExist){
            ...
            if (Image->m_Image->Height > Image->m_Image->Width){
                ...
                if (Image->m_Image->Height > SettingView.m_nImageLimitation){
                    imagex = (imagex * SettingView.m_nImageLimitation) / Image->m_Image->Height;
                }
            }else{
                //X方向の方が大きい
                if (Image->m_Image->Width > SettingView.m_nImageLimitation){
                    imagex = (imagex * SettingView.m_nImageLimitation) / Image->m_Image->Width;
                }
            }
    ...
    if (Drawing) if (m_CardWidth[cardindex] < (SettingView.m_nImageLimitation * imagex) / 100 + Space){
        m_CardWidth[cardindex] = (SettingView.m_nImageLimitation * imagex) / 100 + Space;
    }
    ...
    if (Image) if (m_CardWidth[cardindex] < (Image->m_Image->Width * imagex) / 100 + Space){
        m_CardWidth[cardindex] = (Image->m_Image->Width * imagex) / 100 + Space;
    }
    F->Height = OrgFontHeight;
}
//---------------------------------------------------------------------------
void TFo_Main::DrawCard(TCanvas *C, TCard *Card, int Size, int cardindex, TColor HMColor, int option)
{
    //Y方向座標計算
    int Space = (m_nFontHeight * Size) / 100;
    if (Space < 1){
    ...
    if (Card){
        ...
        if (SettingView.m_bImageVisible && !Drawing && !Video && Card->m_ImageFN != ""){
            Image = SearchImage(Card->m_ImageFN);
        }
    }
...

    {
        ...
        if (!((top < 0 && bottom2 < 0) || (top > m_nBrowserHeight && bottom2 > m_nBrowserHeight)) &&
            !((left < 0 && right2 < 0) || (left > m_nBrowserWidth && right2 > m_nBrowserWidth))){
            //画面内
            ...
            while (drawtimes){
                ...
                if (SettingView.m_bCardGradation && !(option & 0x1)){
                    ...
                    case 2:
                        //下半分
                        MyRgn = ::CreateRectRgn(m_nXOffset, m_nYOffset + m_CardY[cardindex], m_nXOffset + m_nBrowserWidth, m_nYOffset + m_nBrowserHeight);
                        break;
                    case 1:
                        //上半分
                        //B->Color = BrightColor(B->Color, 1.25f);
                        B->Color = HalfColor(B->Color, 0xffffff, 0.25f);
                        MyRgn = ::CreateRectRgn(m_nXOffset, m_nYOffset, m_nXOffset + m_nBrowserWidth, m_nYOffset + m_CardY[cardindex]);
                        break;
                    }
                ...
                case 14://Hex
                    {
                        ...
                        ps[5].x = m_nXOffset + (left + right) / 2 + sin(M_PI / 6 * 11) * Space;
                        ps[5].y = m_nYOffset + (top + bottom) / 2 - cos(M_PI / 6 * 11) * Space;
                        C->Polygon(ps, 5);
                    }
                    break;
                case 15://☆
                    {
                        int c = (Space * sin(M_PI * (18.0 / 360.0))) / sin(M_PI * (54.0 / 360.0));
                        TPoint ps[10];
            ...
            if (Drawing) {
                if ((option & 0x1) == 0){
                    ...
                    MyRgn = ::CreateRectRgn(
                        m_nXOffset + left + Space / 2,
                        m_nYOffset + top + Space / 2,
                        m_nXOffset + left + Space / 2 + (SettingView.m_nImageLimitation * imagex) / 100,
                        m_nYOffset + top + Space / 2 + (SettingView.m_nImageLimitation * imagex) / 100 * 0.75);
                    RgnBak = ::CreateRectRgn(0, 0, 0, 0);
                    ...
                    TRect R = Rect(
                        m_nXOffset + left + Space / 2,
                        m_nYOffset + top + Space / 2 - (SettingView.m_nImageLimitation * imagex) / 100 * 0.125,
                        m_nXOffset + left + Space / 2 + (SettingView.m_nImageLimitation * imagex) / 100,
                        m_nYOffset + top + Space / 2 + (SettingView.m_nImageLimitation * imagex) / 100 * 0.875
                        );
                    ...
                    if (imagex == 100){
                        ...
                        C->StretchDraw(
                            Rect(
                                m_nXOffset + left + Space / 2,
                                m_nYOffset + top + Space / 2,
                                m_nXOffset + left + Space / 2 + (imagex * Image->m_Image->Width) / 100,
                                m_nYOffset + top + Space / 2 + (imagex * Image->m_Image->Height) / 100
                                ),
                            Image->m_Image
                            );
                    }
            ...
            TColor fcbak = F->Color;
            {
                ...
                float Y = m_nYOffset + bottom - Space / 2 - lineheight * lines - tickerheight;
            ...
            if (SettingView.m_bTickerVisible && tickerheight > 0 && ticker){
                ...
                if (Card->m_fTickerPos * m_nFontHeight > tickerwidth + m_CardWidth[cardindex] - Space){
                ...
                if (m_CardShape[cardindex] >= 3 || ((option & 0x1) == 0)){
                    C->TextRect(
                        Rect(m_nXOffset + left + Space / 2, Y, m_nXOffset + right - Space / 2, Y + tickerheight),
                ...
                if (m_CardShape[cardindex] >= 3 || ((option & 0x1) == 0)){
                    if (Card){
                        ...
                        if (Card->m_fScore > 0.0f && m_fMaxScore - m_fMinScore > 0.0f){
                            score = (log(Card->m_fScore) - m_fMinScore) / (m_fMaxScore - m_fMinScore) * 5.0f;
                        }
                        B->Style = bsSolid;
                        if ((option & 0x1) == 0){
                            //影でない
            ...
            //Foldボタン描画
            if (Card) if ((option & 0x1) == 0 && TreeMode() && Card->m_bHasChild){
                P->Style = psSolid;
                B->Style = bsSolid;
                P->Color = PColor;
                P->Width = m_fFontZoom;
    ...
    if (Card && (option & 0x1) == 0){
        //カードがあり、影でない
        if (Card->m_nSelected && Card->m_nID != m_nLastTarget && Card->m_nID != m_nTargetCard){
...
//---------------------------------------------------------------------------
void TFo_Main::DrawFocusCursor(TCanvas *C, float fx, float fy, float fw, float fh)
{
    TPen *P = C->Pen;
...
//---------------------------------------------------------------------------
void TFo_Main::DrawPatternLine(TCanvas *C, int Pattern, int X1, int Y1, int X2, int Y2, int penwidth)
{
    //線の長さ
    float xd = X2 - X1;
    float yd = Y2 - Y1;
...
void TFo_Main::DrawCurvedLine(TCanvas *C, int Pattern, int X1, int Y1, int X2, int Y2, int penwidth, int direction)
{
    //曲線描画
    //線の長さ
    float xd = X2 - X1;
    ...
    while (pos < M_PI){
        ...
        while (pos < M_PI){
            ...
            if ((lastx - xd * x0) * (lastx - xd * x0) + (lasty - yd * y0) * (lasty - yd * y0) > Pat[ppos] * Pat[ppos]) {
                break;
            }
        }
        ppos++;
...
//---------------------------------------------------------------------------
void TFo_Main::DrawLink2(TCanvas *C, TLink *Link, int X1, int Y1, int X2, int Y2, TColor HMColor, int option)
{
    //リンク描画
        ...
        //通常矢印
        {
            ...
            if (option & 0x2){
                ...
                if (barrow && shape < 11){
                    ...
                    float mY = (Y1 + Y2) / 2;
                    if (Y2 != Y1 || X2 != X1){
                        C->MoveTo(mX - cos(rad + 0.1 * 2 * M_PI) * (m_nFontHeight * 2) / 3, mY - sin(rad + 0.1 * 2 * M_PI) * (m_nFontHeight * 2) / 3);
                        C->LineTo(mX, mY);
                        C->MoveTo(mX - cos(rad - 0.1 * 2 * M_PI) * (m_nFontHeight * 2) / 3, mY - sin(rad - 0.1 * 2 * M_PI) * (m_nFontHeight * 2) / 3);
                        C->LineTo(mX, mY);
                    }
    ...
    if (SettingView.m_bLinkNameVisible && Link){
        if (Link->m_Name != ""){
            ...
            AnsiString S = Link->m_Name;
            if (mX > m_nBrowserWidth / 2){
                C->TextOut(mX + m_nFontHeight / 2, mY - m_nFontHeight / 2, S);
            }else{
                C->TextOut(mX - m_nFontHeight / 2 - C->TextWidth(S), mY - m_nFontHeight / 2, S);
            }
        }
    }
}
//---------------------------------------------------------------------------
void TFo_Main::DrawLink(TCanvas *C, TLink *Link, int card1index, int card2index, TColor HMColor, int option)
{
    //リンク描画
...
//---------------------------------------------------------------------------
void TFo_Main::DrawLabelCircleRect(TCanvas *C, bool drawtoporder, bool drawothers)
{
    TBrush *B = C->Brush;
    TPen *P = C->Pen;
    TFont *F = C->Font;
    if (m_Document->m_Labels[0]->Count > 0 &&
        (SettingView.m_bLabelCircleVisible || SettingView.m_bLabelRectangleVisible)){
        //各ラベルのDateを指定
                    ...
                    if ((first && drawtoporder) || (!first && drawothers)){
                        ...
                        if (count){
                            ...
                            if (StDev[0] > 0.0f && StDev[1] > 0.0f){
                                ...
                                //塗りつぶす
                                if (SettingView.m_bLabelFill && (Bu_ArrangeType->Tag % 100 == 3) && Bu_ArrangeType->Tag < 500){
                                    B->Style = bsSolid;
                                ...
                                if (SettingView.m_bLabelNameVisible){
                                    ...
                                    F->Height = (RE_Edit->Font->Height * Label->m_nSize * m_fFontZoom) * 0.01;
                                    F->Style = TFontStyles() << fsBold;
                                    C->TextOut(
                                        Mean[0] + m_nXOffset - C->TextWidth(Label->m_Name) * 0.5,
                                        (Mean[1] - StDev[1] * 2.1f) + m_nYOffset + C->TextHeight(Label->m_Name) * 0.5,
                                        Label->m_Name);
                                }
                ...
                if (Label->m_bEnable && !Label->m_bFold){
                    ...
                    if ((first && drawtoporder) || (!first && drawothers)){
                        ...
                        if (count){
                            ...
                            C->RoundRect(
                                MinX + m_nXOffset - space,
                                MinY + m_nYOffset - space,
                                MaxX + m_nXOffset + space,
                                MaxY + m_nYOffset + space,
                                space,
                                space);
                            ...
                            if (SettingView.m_bLabelNameVisible){
                                ...
                                if (OrgMinY < 0.4f){
                                    C->TextOut(
                                        ((MinX + MaxX) / 2) + m_nXOffset - C->TextWidth(Label->m_Name) * 0.5,
                                        MinY - space - space - C->TextHeight(Label->m_Name) + m_nYOffset,
                                        Label->m_Name);
                                }else{
                                    C->TextOut(
                                        ((MinX + MaxX) / 2) + m_nXOffset - C->TextWidth(Label->m_Name) * 0.5,
...
TColor TFo_Main::GetCardColor(TCard *Card, float &SizeX)
{
    ...
    for (int il = 0 ; il < Card->m_Labels->Count ; il++){
        ...
        if (Label->m_bEnable){
            ...
            SizeX *= m_Document->GetLabelByIndex(0, Card->m_Labels->GetLabel(il) - 1)->m_nSize / 100.0f;
            count ++;
        }
    }
...
void TFo_Main::PaintSub(TCanvas *C)
{
    ...
    if (m_nTmpCardsCount != m_Document->m_Cards->Count){
        ...
        m_CardTitle = new AnsiString[m_Document->m_Cards->Count];
    ...
    float addx = (Zoom * (0.05f - Sc_X->Position * 0.0001f) + 0.5f) * m_nBrowserWidth;
    float addy = (Zoom * (0.05f - Sc_Y->Position * 0.0001f) + 0.5f) * m_nBrowserHeight;
    ...
    //背景塗りつぶし
    if (fabs(m_fFontZoom - 1.0) < 0.1 && !(Im_Wall->Enabled && SettingView.m_bFixWallPaper)){
    ...
    if (Im_Wall->Enabled){
        if (SettingView.m_bFixWallPaper){
            //固定表示
            C->StretchDraw(Rect(m_nXOffset, m_nYOffset, m_nXOffset + m_nBrowserWidth, m_nYOffset + m_nBrowserHeight), Im_Wall->Picture->Graphic);
        }else{
            //スクロール表示
            ...
            for (int iy = starty ; iy <= endy ; iy++){
                int top = ((iy - (Sc_Y->Position - 5000.0f) * 0.0001f - 0.5f) * Zoom + 0.5f) * m_nBrowserHeight;
                int bottom = ((iy + 1 - (Sc_Y->Position - 5000.0f) * 0.0001f - 0.5f) * Zoom + 0.5f) * m_nBrowserHeight;
                if (bottom >= 0 && top < m_nBrowserHeight){
                    for (int ix = startx ; ix <= endx ; ix++){
                        int left = ((ix - (Sc_X->Position - 5000.0f) * 0.0001f - 0.5f) * Zoom + 0.5f) * m_nBrowserWidth;
                        int right = ((ix + 1 - (Sc_X->Position - 5000.0f) * 0.0001f - 0.5f) * Zoom + 0.5f) * m_nBrowserWidth;
    ...
    if (SettingView.m_bTextVisible && m_nTargetCard >= 0){
        ...
        if (SettingView.m_bTextCentering){
            int maxwidth = 0;
            for (int i = 0 ; i < RE_Edit->Lines->Count && y + i * height < m_nBrowserHeight ; i++){
        ...
        for (int i = 0 ; i < RE_Edit->Lines->Count && y + i * height < m_nBrowserHeight ; i++){
            C->TextOut(x + m_nXOffset, y + height * i + m_nYOffset, RE_Edit->Lines->Strings[i]);
        }
    }
    ...
    for (int i2 = 0 ; i2 < m_Document->m_Links->Count ; i2++){
        ...
        if (visible){
            if (LinkCard1Index[i2] >= 0 && LinkCard2Index[i2] >= 0){
                visible &= m_CardVisible[LinkCard1Index[i2]] && m_CardVisible[LinkCard2Index[i2]];
            }else{
                visible = false;
            }
        }
    ...
    for (int i2 = 0 ; i2 < m_Document->m_Links->Count ; i2++) if (m_LinkVisible[i2]){
        ...
        if (Link->m_nFromID != m_nTargetCard && Link->m_nDestID != m_nTargetCard && i2 != m_nTargetLink){
            //無関係なリンク
        }else{
    ...
    for (int i = 0 ; i < m_Document->m_Cards->Count ; i++) if (m_CardVisible[i]){
        ...
        if (Card->m_bLabelIsFold){
            ...
            if (Card->m_nID == m_nTargetCard){
                ...
                m_CardAssign[i] = i;
                for (int i2 = 0 ; i2 < m_Document->m_Cards->Count ; i2++) if (i != i2 && m_CardVisible[i2]){
                ...
                //カードループ
                //後に出てきたこのカードを代表カードに
                for (int i2 = 0 ; i2 < m_Document->m_Cards->Count ; i2++) if (m_CardVisible[i2] && m_CardAssign[i2] == idxbak){
                    m_CardAssign[i2] = i;
                }
            }
        }
    ...
    //カードループ
    for (int i = 0 ; i < m_Document->m_Cards->Count ; i++) if (m_CardVisible[i] && m_CardRelated[i]){
        //関連カード
        TCard *Card = m_Document->GetCardByIndex(i);
        ...
        if (Card->m_bLabelIsFold){
            ...
            //カードループ
            //関連カードを全てRelatedに
            for (int i2 = 0 ; i2 < m_Document->m_Cards->Count ; i2++) if (m_CardAssign[i] == m_CardAssign[i2]){
                m_CardRelated[i2] = true;
            }
        }
    }
    ...
    for (int i = 0 ; i < m_Document->m_Cards->Count ; i++) if (m_CardVisible[i] && m_CardAssign[i] == i){
        ...
        if (Card->m_bLabelIsFold){
            ...
            for (int i2= 0 ; i2 < m_Document->m_Cards->Count ; i2++) if (m_CardVisible[i] && m_CardAssign[i2] == i){
                TCard *Card2 = m_Document->GetCardByIndex(i2);
    ...
    //必要に応じてカードサイズの事前計算
    //影描画
    if ((m_Document->m_Labels[0]->Count > 0 && SettingView.m_bLabelRectangleVisible) || //ラベルを囲む矩形
        ...
        for (int ic = 0 ; ic < m_Document->m_Cards->Count ; ic++){
            ...
            if (m_CardVisible[i] && m_CardAssign[i] == i){
                ...
                if ((ic >= m_Document->m_Cards->Count - 3 && m_bShowRecent) || m_nTargetCard == Card->m_nID){
                    //最近表示したカード強調
                    F->Style = TFontStyles() << fsBold;
                }else{
                    F->Style = TFontStyles();
    ...
    for (int i2 = 0 ; i2 < m_Document->m_Links->Count ; i2++) if (m_LinkVisible[i2]){
        ...
        if (!(m_CardRelated[LinkCard1Index[i2]] && m_CardRelated[LinkCard2Index[i2]])){
            //無関係なリンク
            ...
            if (Link->m_Labels->Count == 0){
                ...
                for (int il = 0 ; il < Link->m_Labels->Count ; il++){
                    TCardLabel *Label = m_Document->GetLabelByIndex(1, Link->m_Labels->GetLabel(il) - 1);
            ...
            if ((Bu_ArrangeType->Tag % 100 > 2 && Bu_ArrangeType->Tag <= 499) ||
                ...
                if (From->m_nParentID != Dest->m_nID && Dest->m_nParentID != From->m_nID){
            ...
            //無関係なリンク
            Link->m_Color = P->Color;
            DrawLink(C, Link, LinkCard1Index[i2], LinkCard2Index[i2], HMColor, linkoption);
        }
    }
    ...
    for (int ic = 0 ; ic < m_Document->m_Cards->Count ; ic++){
        ...
        if (!m_CardRelated[i] && m_CardAssign[i] == i && Card->m_bVisible && Card->m_nID != m_nTargetCard){
            //無関係なカード
    ...
    F->Color = (TColor)HalfColor(m_nFGColor, m_nBGColor, 0.25);
    F->Style = TFontStyles();
    for (int i2 = 0 ; i2 < m_Document->m_Links->Count ; i2++) if (m_LinkVisible[i2] && m_nTargetLink != i2){
        ...
        if (m_CardRelated[LinkCard1Index[i2]] && m_CardRelated[LinkCard2Index[i2]]){
            ...
            if (Link->m_Labels->Count == 0){
                P->Color = (TColor)HalfColor(m_nFGColor, m_nBGColor, 0.125);//(TColor)m_nFGColor;
            }else{
                TColor c = (TColor)0;
                for (int il = 0 ; il < Link->m_Labels->Count ; il++){
                    TCardLabel *Label = m_Document->GetLabelByIndex(1, Link->m_Labels->GetLabel(il) - 1);
            ...
            //関係あるリンク
            Link->m_Color = P->Color;
            TColor HMColor2 = HalfColor(m_nFGColor, m_nBGColor, 0.5);//HalfColor(HMColor, m_nFGColor, 0.5)
            DrawLink(C, Link, LinkCard1Index[i2], LinkCard2Index[i2], HMColor2, linkoption | 0x02);
        }
    }

    P->Color = (TColor)m_nFGColor;
    if (m_bMDownBrowser == 1 && m_nTargetCard >= 0 && (SB_Line->Down || SB_ToolLinkLabel->Down)){
        //リンク線追加中
        ...
        if (m_nTargetCard2 >= 0){
            ...
            float X1 = m_CardX[card1index];
            float Y1 = m_CardY[card1index];
            DrawLink2(C, NULL, X1, Y1, m_nMDownTargetX, m_nMDownTargetY, HMColor, linkoption);
        }
    }
    ...
    if (m_nTargetLink >= 0){
        ...
        if (Link){
            ...
            if (Link->m_Labels->Count == 0){
                ...
                TColor c = (TColor)0;
                for (int il = 0 ; il < Link->m_Labels->Count ; il++){
                    c = HalfColor(c, (TColor)m_Document->GetLabelByIndex(1, Link->m_Labels->GetLabel(il) - 1)->m_nColor, 1.0f / (il + 1));
                }
                P->Color = c;
            }
            ...
            if (!m_bMDownBrowserMoved || m_bMDownBrowser <= 1){
                ...
                TColor HMColor2 = HalfColor(m_nFGColor, m_nBGColor, br);//HalfColor(HMColor, m_nFGColor, 0.5)
                DrawLink(C, Link, LinkCard1Index[m_nTargetLink], LinkCard2Index[m_nTargetLink], HMColor2, linkoption | 0x2);
            }else switch (m_bMDownBrowser){
            case 2:
                //Dest変更中
                if (m_nTargetCard2 >= 0){
                    //相手がいる
                    int index = m_Document->SearchCardIndex(m_nTargetCard2);
                    DrawLink(C, Link, LinkCard1Index[m_nTargetLink], index, HMColor, linkoption);
                }else{
                    //相手がいない
                    float X1 = m_CardX[LinkCard1Index[m_nTargetLink]];
                    float Y1 = m_CardY[LinkCard1Index[m_nTargetLink]];
                    DrawLink2(C, Link, X1, Y1, m_nMDownTargetX, m_nMDownTargetY, HMColor, linkoption);
                }
                break;
            case 3:
                //From変更中
                if (m_nTargetCard2 >= 0){
                    //相手がいる
                    int index = m_Document->SearchCardIndex(m_nTargetCard2);
                    DrawLink(C, Link, index, LinkCard2Index[m_nTargetLink], HMColor, linkoption);
                }else{
                    //相手がいない
                    float X1 = m_CardX[LinkCard2Index[m_nTargetLink]];
                    float Y1 = m_CardY[LinkCard2Index[m_nTargetLink]];
                    DrawLink2(C, Link, m_nMDownTargetX, m_nMDownTargetY, X1, Y1, HMColor, linkoption);
                }
                break;
            }
        }
    ...
    for (int ic = 0 ; ic < m_Document->m_Cards->Count ; ic++){
        ...
        if (m_CardRelated[i] && m_CardAssign[i] == i && Card->m_nID != m_nTargetCard && Card->m_bVisible){
            //関係のあるカード
            P->Width = (int)(3 * m_fFontZoom);
            if (ic >= m_Document->m_Cards->Count - 3 && m_bShowRecent){
                //最近表示したカード強調
    ...
    */
    ...
    //Focusを示すカーソル
    if (SettingView.m_bFocusCursor && (m_nTargetCard >= 0 || m_nLastTarget >= 0)){
        ...
        if (current >= 0 || last >= 0){
            ...
            if (pos >= 0){
                ...
                if (last >= 0 && current >= 0){
                    ...
                    fw = m_CardWidth[last] * (1.0f - ratio) + m_CardWidth[current] * ratio;
                    fh = m_CardHeight[last] * (1.0f - ratio) + m_CardHeight[current] * ratio;
                }else if (last >= 0){
                    //直前のカードのみある
                    fx = m_CardX[last];
                    fy = m_CardY[last];
    ...
    if (m_bMDownBrowser == 5){
        ...
        P->Color = TColor(m_nFGColor);
        C->RoundRect(
            m_nXOffset + m_nMDownBrowserX * m_fFontZoom,
            m_nYOffset + m_nMDownBrowserY * m_fFontZoom,
            m_nXOffset + m_nMDownTargetX * m_fFontZoom,
            m_nYOffset + m_nMDownTargetY * m_fFontZoom,
            m_nFontHeight,
            m_nFontHeight);
    }
    ...
    if (SettingView.m_bOverview){
        ...
        P->Width = m_fFontZoom;
        P->Style = psSolid;
        C->Rectangle(m_nOVXOffset, m_nOVYOffset, m_nOVXOffset + m_nOVWidth, m_nOVYOffset + m_nOVHeight);

        HRGN MyRgn;
        MyRgn = ::CreateRectRgn(m_nOVXOffset, m_nOVYOffset, m_nOVXOffset + m_nOVWidth, m_nOVYOffset + m_nOVHeight);
        ::SelectClipRgn(C->Handle, MyRgn);

        for (int i = 0 ; i < m_Document->m_Cards->Count ; i++) if (m_CardVisible[i]){
            TCard *Card = m_Document->GetCardByIndex(i);
            int x = m_nOVXOffset + ((Card->m_fX - 0.5f) * OVERVIEWSIZE + 0.5f) * m_nOVWidth;
            int y = m_nOVYOffset + ((Card->m_fY - 0.5f) * OVERVIEWSIZE + 0.5f) * m_nOVHeight;
            P->Color = Card->m_Color;
            B->Color = Card->m_Color;
            C->FillRect(Rect(x - 2 * m_fFontZoom, y - 1 * m_fFontZoom, x + 2 * m_fFontZoom, y + 1 * m_fFontZoom));
        ...
        bottom = m_nOVYOffset + ((bottom - 0.5f) * OVERVIEWSIZE + 0.5f) * m_nOVHeight;
    ...
    //Demo String
    if (m_DemoString != ""){
        C->TextOut(8, m_nBrowserHeight - 8 - C->TextHeight(m_DemoString), m_DemoString);
    }
    ...
    for (int il_ = 0 ; il_ < m_Document->m_Labels[0]->Count ; il_++){
        ...
        if (Label->m_bEnable && !Label->m_bFold){
            ...
            for (int ic = 0 ; ic < m_Document->m_Cards->Count ; ic++){
                ...
                if (!m_CardRelated[i] && m_CardAssign[i] == i && Card->m_bVisible && Card->m_nID != m_nTargetCard && Card->m_Labels->Contain(il + 1)){
                    //無関係なカード
                    if (ic >= m_Document->m_Cards->Count - 3 && m_bShowRecent){
                        //最近表示したカード強調
                        F->Style = TFontStyles() << fsBold;
...
bool TFo_Main::BrowserAutoScroll()
{
    ...
    if (SB_AutoScroll->Down || m_nScrollTargetX > -65536){
        ...
        int xp = Sc_X->Position * (1.0f - m_fBrowserScrollRatio) + AX * m_fBrowserScrollRatio;
        int yp = Sc_Y->Position * (1.0f - m_fBrowserScrollRatio) + AY * m_fBrowserScrollRatio;
    ...
    if (SB_AutoZoom->Down){
        ...
        for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
            ...
            if (m_CardVisible[i] &&
                (m_CardRelated[i] ||
                 ...
                 (m_bShowRecent && Card->m_nViewedOrder >= m_Document->m_Cards->Count - 3) ||
                 Card->m_bSelected ||
        ...
        if (count && XD > 0.0f && YD > 0.0f){
            ...
            if (XD > 0.0f){
                ...
                int zp = (int)(TB_Zoom->Position * (1.0f - zoomratio) + targetzoom * zoomratio);
                /*
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    if (PC_Client->ActivePage != TS_Browser || m_bDblClicked || m_CardVisible == NULL){
        return;
    }
    ...
    */
        ...
        if (Button == mbLeft){
            if (!Card->m_bLabelIsFold){
                ...
                if (Shift.Contains(ssShift) && Shift.Contains(ssCtrl)){
                    ...
                    bool b = true;
                    if (SettingFile.fepOperation){
                        b = !SettingFile.fepOperation(m_Document, "Card_EditTitle");
                    }
                ...
                for (int i2 = 0 ; i2 < m_Document->m_Cards->Count ; i2++) if (m_CardVisible[i2] && i != i2){
                    TCard *Card2 = m_Document->GetCardByIndex(i2);
                    if (m_Document->LabelIsSame(Card, Card2)){
    ...
    if (m_nTargetCard == -1 && m_Document->m_Links->Count > 0){
        ...
        //カードが選択されていない
        for (int i = 0 ; i < m_Document->m_Links->Count ; i++) if (m_LinkVisible[i]){
            TLink *Link = m_Document->GetLinkByIndex(i);
                ...
                if (Link->m_nShape < 6) {
                    ...
                    if (len > 0.0f) {
                        ...
                        while (pos < M_PI){
                            float x0, y0;
                            CurvePosToXY(pos, x0, y0);
                            double S1 = DistanceToLine(X, Y, X1 + lastx, Y1 + lasty, X1 + xd * x0, Y1 + yd * y0);
                            lastx = xd * x0;
                            lasty = yd * y0;
                            pos += radstep;
                            if (S1 < S) {
...
void TFo_Main::BrowserNewCard()
{
    ...
    Card->m_bSelected = true;
    float cx, cy;
    DispPosToCardPos(m_nMDownTargetX, m_nMDownTargetY,
        cx, cy);
    Card->m_fX = cx;
    Card->m_fY = cy;
...
void __fastcall TFo_Main::PB_BrowserDblClick(TObject *Sender)
{
    ...
    //ダブルクリックのフック
    if (SettingFile.fepOperation){
        if (SettingFile.fepOperation(m_Document, "Card_DblClick")){
            return;
        }
    }
    ...
    if (m_nTargetCard >= 0){
        ...
        if (Card->m_bLabelIsFold){
            //折りたたまれている
            for (int il = 0 ; il < Card->m_Labels->Count ; il++){
                m_Document->GetLabelByIndex(0, Card->m_Labels->GetLabel(il) - 1)->m_bFold = false;
            }
        }else{
            //カードをダブルクリック（エディタを開く）
      ...
      TShiftState Shift, int X, int Y)
{
    ...
    case 1:
        //通常のドラッグ
        if (m_nMDownTargetX != m_nMDownBrowserX || m_nMDownTargetY != m_nMDownBrowserY){
            CloseEditBox();
        }
        if (m_nTargetCard >= 0){
            if (SB_Cursor->Down){
                ...
                float CardY;
                DispPosToCardPos(
                    X + m_nMDownBrowserOffsetX,
                    Y + m_nMDownBrowserOffsetY,
                    CardX, CardY);
                ...
                if (Card){
                    ...
                    if (TreeMode()){
                        ...
                        for (int i = 0 ; i < moved->Count ; i++){
                            ...
                            bool first = true;
                            //カードループ
                            for (int ic = 0 ; ic < m_Document->m_Cards->Count ; ic++) if (m_CardVisible[ic]){
                                TCard *Card2 = m_Document->GetCardByIndex(ic);
                                if (Card != Card2 && Card2->m_nParentID == Card->m_nParentID){
                                    //同じ親を持つカード
                                    if (first){
                                        x = Card2->m_fX;
                                        first = false;
                                    }else if (fabs(Card2->m_fX - Card->m_fX) < fabs(x - Card->m_fX)){
                                        //よりX座標が今のカードに近い
                                        x = Card2->m_fX;
                                    }
                                }
                ...
                //リンク線
                m_nTargetCard2 = - 1;
                for (int i = m_Document->m_Cards->Count - 1 ; i >= 0 ; i--) if (m_Document->GetCardByIndex(i)->m_nID != m_nTargetCard && m_CardVisible[i]){
    ...
    case 2:
        ...
        if (m_nTargetLink >= 0){
            ...
            m_nTargetCard2 = - 1;
            for (int i = m_Document->m_Cards->Count - 1 ; i >= 0 ; i--) if (m_Document->GetCardByIndex(i)->m_nID != Link->m_nFromID && m_CardVisible[i]){
                if (X >= m_CardX[i] - m_CardWidth[i] / 2 &&
                    X < m_CardX[i] + m_CardWidth[i] / 2 &&
                    Y >= m_CardY[i] - m_CardHeight[i] / 2 &&
                    Y < m_CardY[i] + m_CardHeight[i] / 2){
    ...
    case 3:
        ...
        if (m_nTargetLink >= 0){
            ...
            m_nTargetCard2 = - 1;
            for (int i = m_Document->m_Cards->Count - 1 ; i >= 0 ; i--) if (m_Document->GetCardByIndex(i)->m_nID != Link->m_nDestID && m_CardVisible[i]){
                if (X >= m_CardX[i] - m_CardWidth[i] / 2 &&
                    X < m_CardX[i] + m_CardWidth[i] / 2 &&
                    Y >= m_CardY[i] - m_CardHeight[i] / 2 &&
                    Y < m_CardY[i] + m_CardHeight[i] / 2){
    ...
    case 4:
        ...
        {
            //カード座標に変換
            float cx = (((X * 1.0f - m_nOVXOffset) / m_nOVWidth - 0.5f) / OVERVIEWSIZE + 0.5f);
            float cy = (((Y * 1.0f - m_nOVYOffset) / m_nOVHeight - 0.5f) / OVERVIEWSIZE + 0.5f);
    ...
    case 5:
        ...
        if (m_CardVisible) for (int i = 0 ; i < m_Document->m_Cards->Count ; i++) if (m_CardVisible[i]){
            if (m_CardX[i] - m_CardWidth[i] / 2 >= leftx &&
                ...
                if (Card->m_bLabelIsFold){
                    for (int i2 = 0 ; i2 < m_Document->m_Cards->Count ; i2++) if (m_CardVisible[i2] && i2 != i){
                        TCard *Card2 = m_Document->GetCardByIndex(i2);
                        if (m_Document->LabelIsSame(Card, Card2)){
    ...
    if (SB_Arrange->Down){
        ...
        for (int i = 0 ; i < m_Document->m_Links->Count ; i++){
            ...
            if (Link->m_nDestID == lastcard || Link->m_nFromID == lastcard){
                ...
                r +=
                    sqrt(
                        (Card->m_fX - LastCard->m_fX) * (Card->m_fX - LastCard->m_fX) +
        ...
        while (itarcount++ < 100){
            ...
            for (int i = 0 ; i < m_Document->m_Cards->Count && !found; i++){
                ...
                if (Card != NewCard &&
                    ...
                    (Card->m_fY - NewCard->m_fY) * (Card->m_fY - NewCard->m_fY) < (rbak * rbak / 4.0f)){
                    found = true;
                }
            }
            if (!found){
        ...
        if (SB_Arrange->Down){
            ...
            for (int i = 0 ; i < m_Document->m_Links->Count ; i++){
                ...
                if (Link->m_nDestID == ParentID || Link->m_nFromID == ParentID){
                    ...
                    r +=
                        sqrt(
                            (Card->m_fX - ParentCard->m_fX) * (Card->m_fX - ParentCard->m_fX) +
                            (Card->m_fY - ParentCard->m_fY) * (Card->m_fY - ParentCard->m_fY)
                        );
                    count++;
                }
            }
...
void TFo_Main::BrowserInsertCardToLink()
{
    ...
    for (int il = 0 ; il < m_Document->m_Links->Count ; il++) if (il != m_nTargetLink){
        TLink *Link = m_Document->GetLinkByIndex(il);
        if (Link->m_nFromID == lastcard){
            int index = m_Document->SearchCardIndex(Link->m_nDestID) + 1;
            if (index > insindex){
      ...
      TShiftState Shift)
{
    ...
    if (Key >= 37 && Key <= 40 && PC_Client->ActivePage == TS_Browser && m_CardVisible && TB_Zoom->Focused()){
    //if (Key >= 37 && Key <= 40 && PC_Client->ActivePage == TS_Browser && m_CardVisible && (TB_Zoom->Focused() || !Active || Fo_FullScreen->Visible)){
        ...
        if (!Shift.Contains(ssCtrl)){
            ...
            if (!Shift.Contains(ssShift) || m_nTargetCard < 0){
                ...
                for (int i = 0 ; i < m_Document->m_Links->Count ; i++) if (m_LinkVisible[i]){
                    ...
                    if (index >= 0){
                        ...
                        if (Card->m_bGetFocus){
                            float md = MoveDistance(sx, sy, m_CardX[index], m_CardY[index], Key - 37);
                            if (md > 0.0f && (mindist < 0.0f || md < mindist)){
                                mindist = md;
                                minindex = index;
                            }
            ...
            case 39://反時計回りに一番近い兄弟ノードを探す
                {
                    int card = m_Document->SearchBrother(m_nTargetCard, parent, Key == 39, false, true);
                    if (card >= 0){
                        m_nTargetCard = card;
                        m_Document->ClearCardSelection();
    ...
    if (Key == 8 && PC_Client->ActivePage == TS_Browser && m_CardVisible && TB_Zoom->Focused()){
    ...
    if (PC_Client->ActivePage == TS_Browser && TB_Zoom->Focused() &&  
        ((Key >= 48 && Key <= 90) || (Key >= 96 && Key <= 111) || (Key >= 186 && Key <= 226)) &&
        !Ed_TitleB->Visible && !m_bTitleEditRequested &&
        !Shift.Contains(ssAlt) && !Shift.Contains(ssCtrl)//!Shift.Contains(ssShift) &&
        ){
...
void __fastcall TFo_Main::MVF_MagnifyClick(TObject *Sender)
{
    ...
    if (UD_Size->Position > SettingView.m_Font->Size && last < SettingView.m_Font->Size){
        UD_Size->Position = SettingView.m_Font->Size;
    }
...
void __fastcall TFo_Main::ME_FindClick(TObject *Sender)
{
    ...
    Fo_Search->Ch_GlobalSearch->Checked = Sender != (TObject*)ME_FindNext && Sender != (TObject*)ME_FindPrevious;
}
//---------------------------------------------------------------------------
        ...
        if (!Card->m_bVisible && !(SearchRequest & 0x20)){
            ...
            if (SearchRequest & 0x1){
                ...
                while (true){
                    ...
                    if (foundat < lastpos) {
                        ...
                        S = S.SubString(1, foundat - 1) + ReplaceKeyword + S.SubString(foundat + SearchKeyword.Length(), S.Length());
                ...
                if (S != Card->m_Title) {
                    ...
                    m_Document->SetCardTitle(Card, AnsiString(S).c_str());
                }
            }
            if (SearchRequest & 0x2){
                ...
                if (S != Card->m_Lines->Text) {
                    ...
                    m_Document->SetCardText(Card, AnsiString(S).c_str());
                }
            }
        }
    }
...
void TFo_Main::Search(int SearchRequest)
{
    ...
    if (RE_Edit->Focused()){
        //文章編集中
        StartTarget = 1;
        if ((SearchRequest & 0x100) && ((GetSelText() == SearchKeyword) || ((SearchRequest & 0x10) && (WideLowerCase(GetSelText()) == WideLowerCase(SearchKeyword))))) {
            //置換
            BackupSub(ME_Replace->Caption);
            WideString S = RE_Edit->Text;
            int selstartbak = RE_Edit->SelStart;
            RE_Edit->Text = S.SubString(1, RE_Edit->SelStart) + ReplaceKeyword + S.SubString(1 + RE_Edit->SelStart + RE_Edit->SelLength, S.Length());
            RE_Edit->SelStart = selstartbak;
        ...
        if ((SearchRequest & 0x100) && ((GetSelText() == SearchKeyword) || ((SearchRequest & 0x10) && (WideLowerCase(GetSelText()) == WideLowerCase(SearchKeyword))))) {
            ...
            int selstartbak = Ed_Title->SelStart;
            Ed_Title->Text = S.SubString(1, Ed_Title->SelStart) + ReplaceKeyword + S.SubString(1 + Ed_Title->SelStart + Ed_Title->SelLength, S.Length());
            Ed_Title->SelStart = selstartbak;
    ...
    while (true){
        ...
        if (!Card->m_bVisible && !(SearchRequest & 0x20)){
            ...
            if (SearchRequest & 0x2){
                ...
                if (foundat >= 0){
                    ...
                    RE_Edit->SelLength = SearchKeyword.Length();//AnsiString(SearchKeyword).Length();
                    Ed_Title->SelLength = 0;
                    Found = true;
                    break;
                }
        ...
        if (Target == 1 && (SearchRequest & 0x80)){
            //後ろ向きサーチ
            //前のカードを選択
            CardIndex = (CardIndex + m_Document->m_Cards->Count - 1) % m_Document->m_Cards->Count;
...
void __fastcall TFo_Main::ME_WebSearchClick(TObject *Sender)
{
    ...
    AnsiString S =
        Setting2Function.m_WebSearch->Values[Setting2Function.m_WebSearch->Names[MI->Tag]] +
        HTTPEncode(UTF8Encode(Key));
        /*
        AnsiString("\"") +
        HTTPEncode(UTF8Encode(RE_Edit->SelText)) +
        AnsiString("\"");
        */
    AnsiString S2 = "";
    ShellExecute(Handle, NULL,
        S.c_str(),
        S2.c_str(), NULL, SW_SHOW);

    //ショートカット更新
    SettingFile.m_WebSearch = Setting2Function.m_WebSearch->Names[MI->Tag];
    for (int i = 0 ; i < Setting2Function.m_WebSearch->Count ; i++){
        TMenuItem *MI = (TMenuItem*)MI_WebSearch->Items[i];
        if (SettingFile.m_WebSearch == Setting2Function.m_WebSearch->Names[MI->Tag]){
            MI->ShortCut = TextToShortCut("Ctrl+W");
        }else{
            MI->ShortCut = 0;
        }
...
{
    BackupSub(MD_SortCard->Caption + AnsiString(" ") + ((TMenuItem*)Sender)->Caption);
...
//---------------------------------------------------------------------------
void TFo_Main::LinkLimitationSub(int targetcard, int targetlink, int linkdepth, int *Score, bool linkdirection, bool linkbackward)
{
...
void TFo_Main::SetCardVisible(bool bFoldTree)
{
    ...
    //リンク数でフィルタリング
    if (SettingView.m_bLinkLimitation && (m_nTargetCard >= 0 || m_nTargetLink >= 0)){
        ...
        if (SettingView.m_nLinkTarget >= 0){
            ...
            if (idx2 >= 0){
                if (Score[idx2] > 0){
                    ...
                    LinkLimitationSub(
                        SettingView.m_nLinkTarget, -1, SettingView.m_nLinkLimitation, Score2,
                        SettingView.m_bLinkDirection, !SettingView.m_bLinkBackward);
                    for (int ic = 0 ; ic < m_Document->m_Cards->Count ; ic++){
                        /*
                        //カードタイトルに反映してデバッグ
                        ...
                        */
                        if (Score2[ic] == 0){
                            Score[ic] = 0;
                        }else if (SettingView.m_nLinkLimitation < 11 && Score[ic] + Score2[ic] > SettingView.m_nLinkLimitation + 2){
                            Score[ic] = 0;
                        }
                    }
                    delete[] Score2;
...
void TFo_Main::FilteringCard()
{
        ...
        if (Card->m_bVisible){
            if (Card->m_Labels->Count){
                ...
                for (int il = 0 ; il < Card->m_Labels->Count && !visible ; il++){
                    visible |= m_Document->GetLabelByIndex(0, Card->m_Labels->GetLabel(il) - 1)->m_bShow;
                }
        ...
        if (Card->m_bVisible){
            if (Card->m_Labels->Count){
                for (int il = 0 ; il < Card->m_Labels->Count && Card->m_bVisible ; il++){
                    Card->m_bVisible &= !m_Document->GetLabelByIndex(0, Card->m_Labels->GetLabel(il) - 1)->m_bHide;
                }
            }
        }
    }
    ...
    //サイズ
    if (SettingView.m_bSizeLimitation) for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
        TCard *Card = m_Document->GetCardByIndex(i);
        if (Card->m_bVisible){
            float size = Card->m_nSize;
            if (Card->m_Labels->Count){
                for (int il = 0 ; il < Card->m_Labels->Count ; il++){
                    size *= m_Document->GetLabelByIndex(0, Card->m_Labels->GetLabel(il) - 1)->m_nSize * 0.01f;
                }
            }
    ...
    if (SettingView.m_bDateLimitation){
        int currentdate = -2;
        int daterange = (SettingView.m_nDateLimitation * m_Document->m_Cards->Count) / 100;
        if (m_nTargetCard >= 0){
            TCard *Card = m_Document->GetCard(m_nTargetCard);
            switch (SettingView.m_DateLimitationDateType){
            case 0: currentdate = Card->m_nCreatedOrder; break;
            case 1: currentdate = Card->m_nUpdatedOrder; break;
            case 2: currentdate = Card->m_nViewedOrder; break;
            }
        }
        for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
            TCard *Card = m_Document->GetCardByIndex(i);
            if (Card->m_bVisible){
                float date;
                switch (SettingView.m_DateLimitationDateType){
                case 0: date = Card->m_nCreatedOrder; break;
                case 1: date = Card->m_nUpdatedOrder; break;
                case 2: date = Card->m_nViewedOrder; break;
                }
                ...
                case 3:
                    ...
                    if (currentdate > -1.0f){
                        Card->m_bVisible &=
                            date <= currentdate &&
                            fabs(date - currentdate) < daterange;
                    }
                    break;
                case 4:
                    //Newer
...
void __fastcall TFo_Main::Bu_Label0Click(TObject *Sender)
{
    ...
    if (m_nTargetCard < 0){
        ...
        if (index >= Card->m_Labels->Count){
            ...
            //ラベルあり
            ((TMenuItem*)MI_Labels->Items[Card->m_Labels->GetLabel(index) - 1])->Checked = true;
        }
    }
    Po_Label->Tag = index;
...
void __fastcall TFo_Main::MH_ContentsClick(TObject *Sender)
{
    ...
    AnsiString S =
        AnsiString(ExtractFilePath(ParamStr(0))) +
        AnsiString("editor.exe");
    AnsiString S2 =
        AnsiString("\"") +
        AnsiString(ExtractFilePath(ParamStr(0))) +
        AnsiString("help.fip\"");
...
void __fastcall TFo_Main::MH_FrieveSiteClick(TObject *Sender)
{
    ...
    ShellExecute(Handle, NULL, AppURL, NULL, NULL,SW_SHOW);
}
//---------------------------------------------------------------------------
...
void __fastcall TFo_Main::MH_FIPSiteClick(TObject *Sender)
{
    ...
    ShellExecute(Handle, NULL, SoftURL, NULL, NULL,SW_SHOW);
}
//---------------------------------------------------------------------------
...
{
    OD->FileName = SettingFile.m_RecentFolders[((TMenuItem*)Sender)->Tag] + "*.fip";
    MF_OpenClick(Sender);
}
//---------------------------------------------------------------------------
...
void __fastcall TFo_Main::Bu_LinkLabel0Click(TObject *Sender)
{
    ...
    if (m_nTargetLink < 0){
        ...
        if (index >= Link->m_Labels->Count){
            ...
            //ラベルあり
            ((TMenuItem*)MI_LinkLabels->Items[Link->m_Labels->GetLabel(index) - 1])->Checked = true;
        }
    }
...
void TFo_Main::Animation_RandomCard()
{
    ...
    if (!SettingView.m_bAnimationPaused && m_nAnimationCount >= 3000 * pow(2.0, -(SettingView.m_nAnimationRCSpeed - 30) / 25.0)){
        m_nAnimationCount = 0;
    }
}
//---------------------------------------------------------------------------
...
void TFo_Main::Animation_RandomMap()
{
    ...
    if (m_nAnimationCount == 0){
        ...
        if (m_nTargetCard >= 0){
            ...
            for (int i = 0 ; i < m_Document->m_Links->Count && count < SettingView.m_nAnimationRCCards ; i++){
                TLink *Link = m_Document->GetLinkByIndex(i);
                if (Link->m_bVisible){//表示対象のリンク
                    ...
                    if (m_CardVisible[fromidx]){//表示中=表示対象
                        ...
                        if (!m_CardVisible[destidx] && Dest->m_bVisible){//表示されておらず、表示対象
                            CVNext[destidx] = true;
                            LVNext[i] = true;
    ...
    if (!SettingView.m_bAnimationPaused && m_nAnimationCount >= 2000 * pow(2.0, -(SettingView.m_nAnimationRCSpeed - 30) / 25.0)){
        m_nAnimationCount = 0;
    }
}
//---------------------------------------------------------------------------
...
void TFo_Main::Animation_RandomCard2()
{
    ...
    for (int i = 0 ; i < MAXRANDOMCARDS ; i++){
        if (i < SettingView.m_nAnimationRCCards){
            ...
            if (m_Animation_RC2Idxs[i] < 0){
                ...
                //表示可能なカードをフィルタリング
                for (int i2 = 0 ; i2 < m_Document->m_Cards->Count ; i2++) if (m_CardAssign[i2] == i2){
                ...
                for (int i2 = 0 ; i2 < m_Document->m_Cards->Count ; i2++){
                    ...
                    if (Card->m_bVisible){
                        bool found = false;
                        for (int i3 = 0 ; i3 < SettingView.m_nAnimationRCCards && !found; i3++){
                    ...
                    if (idx >= 0){
                        ...
                        */
                        ...
                        case 0://上下から
                            ...
                            m_Animation_RC2AXs[i] = -(Card->m_fX - 0.5f) + 0.5f + (rand() % 10001) / 10000.0f - 0.5f;
                            if (direction == 0){
                                //上
                                Card->m_fY = 2.0f;
                        ...
                        case 1://左右から
                            ...
                            m_Animation_RC2AYs[i] = -(Card->m_fX - 0.5f) + 0.5f + (rand() % 10001) / 10000.0f - 0.5f;
                            if (direction == 3){
                                //左
                                Card->m_fX = 2.0f;
    ...
    for (int i = 0 ; i < SettingView.m_nAnimationRCCards ; i++){
        if (m_Animation_RC2Idxs[i] >= 0){
            ...
            if (Card->m_fX < -1.0f || Card->m_fY < -1.0f ||
                Card->m_fX > 2.0f || Card->m_fY > 2.0f){
                //画面外にはみ出した
                m_Animation_RC2Idxs[i] = -1;
            }else{
                Card->m_bVisible = true;
...
void TFo_Main::Animation_RandomTrace()
{
    ...
    if (m_nAnimationCount == 0){
        ...
        if (m_nTargetCard >= 0){
            ...
            for (int il = 0 ; il < m_Document->m_Links->Count ; il++) if (m_LinkVisible[il]){
                TLink *Link = m_Document->GetLinkByIndex(il);
                if ((Link->m_nFromID == m_nTargetCard && Link->m_nDestID != m_nAnimation_LastCard) ||
                    (Link->m_nDestID == m_nTargetCard && Link->m_nFromID != m_nAnimation_LastCard && !Link->m_bDirection)){
                    //辿れるリンク
                    count++;
                }
            }
    ...
    if (!SettingView.m_bAnimationPaused && m_nAnimationCount >= 1000 * pow(2.0, -(SettingView.m_nAnimationRCSpeed - 30) / 25.0)){
        m_nAnimationCount = 0;
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TFo_Main::PL_CardPropertyClick(TObject *Sender)
{
    ...
    if (Card){
        ...
        //Title
        S +=
            MLText.Title + AnsiString(" : ") + Card->m_Title;
        S += ret;
        ...
        //Label
        if (Card->m_Labels->Count){
            S += MLText.Labels + AnsiString(" : ");
            for (int i = 0 ; i < Card->m_Labels->Count ; i++){
                if (i > 0){
                    S += ", ";
                }
                S += m_Document->GetLabelByIndex(0, Card->m_Labels->GetLabel(i) - 1)->m_Name;
            }
            S += ret;
        }else{
            S += MLText.Labels + AnsiString(" : 0");
            S += ret;
        }
        ...
        S += MLText.Links + AnsiString(" : ") + IntToStr(count);
        S += ret;

        S += ret;

        S += MLText.Size + AnsiString(" : ") + SizeToStr(Card->m_Lines->Text.Length());
        S += ret;

        S += ret;

        //Date
        S += MLText.Created + AnsiString(" : ") + DateTimeToStr(Card->m_fCreated);
        S += ret;
        S += MLText.Edited + AnsiString(" : ") + DateTimeToStr(Card->m_fUpdated);
        S += ret;
        S += MLText.Viewed + AnsiString(" : ") + DateTimeToStr(Card->m_fViewed);


        Application->MessageBox(S.c_str(), DeleteActionKey(PL_CardProperty->Caption).c_str(), 0);
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TFo_Main::RE_EditMouseDown(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if (Shift.Contains(ssCtrl)){
        //Ctrl+クリック
        ...
        if (line < RE_Edit->Lines->Count){
            AnsiString Str = RE_Edit->Lines->Strings[line];
...
//---------------------------------------------------------------------------
void TFo_Main::OpenExtLink(AnsiString S)
{
...
{
    if (OD_Txt->Execute()){
        BackupSub(MF_Import->Caption + AnsiString(" ") + MFI_TxtFile->Caption);
...
void __fastcall TFo_Main::MFE_TxtFilesClick(TObject *Sender)
{
    ...
    SD_Txt->FileName = ForFileName(m_Document->GetCardByIndex(0)->m_Title) + ".txt";
    if (SD_Txt->Execute()){
        for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
            TCard *Card = m_Document->GetCardByIndex(i);
            TFileStream *FS = new TFileStream(
                ExtractFilePath(SD_Txt->FileName) + ForFileName(Card->m_Title) + ".txt", fmCreate);
            Card->m_Lines->SaveToStream(FS);
            delete FS;
        }
    }
    ...
    if (SD_Html->Execute()){
        //index.html
        {
            ...
            try{
                ...
                ShowMessage(MLText.FailedToSave + SD_Html->FileName + AnsiString("."));
            }
            delete SL;
        }
        ...
        //cards.html
        {
            ...
            SL->Add("<head>");
            SL->Add(
                AnsiString("<title>") + DocumentTitle + "</title>");
            SL->Add("<link rel=StyleSheet href=\"./style.css\">");

            SL->Add("</head>");
            SL->Add("<body>");
            SL->Add(
                AnsiString("<font size=+2>") + DocumentTitle +  "</font>");
            SL->Add("<hr>");
            for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
                TCard *Card = m_Document->GetCardByIndex(i);
                SL->Add(
            ...
            try{
                SL->SaveToFile(
                    ExtractFilePath(SD_Html->FileName) + "cards.html");
            }
            catch(...){
                ShowMessage(MLText.FailedToSave + ExtractFilePath(SD_Html->FileName) + "cards.html" + AnsiString("."));
            }
            delete SL;
        }
        ...
        for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
            ...
            SL->Add("<head>");
            SL->Add(
                AnsiString("<title>") + DocumentTitle + Card->m_Title + "</title>");
            SL->Add("<link rel=StyleSheet href=\"./style.css\">");

            SL->Add("</head>");
            SL->Add("<body>");
            SL->Add(
                AnsiString("<font size=+2>") + DocumentTitle + Card->m_Title + "</font>");

            //前のカード、次のカード
            SL->Add("<hr>");
            SL->Add(
                AnsiString("<a href=\"./1.html\">[Top]</a> "));
            if (i > 0){
                SL->Add(
                    AnsiString("<a href=\"./") +
                    IntToStr(i - 1 + 1) +
            ...
            try{
                SL->SaveToFile(
                    ExtractFilePath(SD_Html->FileName) + IntToStr(i + 1) + ".html");
            }
            catch(...){
                ShowMessage(MLText.FailedToSave + ExtractFilePath(SD_Html->FileName) + IntToStr(i + 1) + ".html" + AnsiString("."));
            }
            delete SL;
        }
    }
...
void __fastcall TFo_Main::PE_CutToNewCardTitleClick(TObject *Sender)
{
    ...
    for (int i = 0 ; i < Me->Lines->Count ; i++){
        AnsiString S = Trim(Me->Lines->Strings[i]);
        if (S != ""){
            //カードの新規作成
            MI_NewCardClick(Sender);
...
void __fastcall TFo_Main::RE_EditMouseUp(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    RefreshLaStatus();
}
...
void __fastcall TFo_Main::MF_PrintImageClick(TObject *Sender)
{
    ...
    if (PD_Image->Execute()){
        ...
        for (int i = 0 ; i < PD_Image->Copies ; i++){
            HRGN MyRgn;
            MyRgn = ::CreateRectRgn(printr.left, printr.top, printr.right, printr.bottom);
...
void TFo_Main::ApplyLanguageSetting()
{
    ...
    //メインウインドウ
    TS_Browser->Caption = Ini->ReadString("MainWnd", "TS_Browser", TS_Browser->Caption);//ブラウザ
    TS_Editor->Caption = Ini->ReadString("MainWnd", "TS_Editor", TS_Editor->Caption);//エディタ
    TS_Drawing->Caption = Ini->ReadString("MainWnd", "TS_Drawing", TS_Drawing->Caption);//図
    TS_Statistics->Caption = Ini->ReadString("MainWnd", "TS_Statistics", TS_Statistics->Caption);//統計
    La_Files->Caption = Ini->ReadString("MainWnd", "La_Files", La_Files->Caption);//ファイル
    La_Cards->Caption = Ini->ReadString("MainWnd", "La_Cards", La_Cards->Caption);//カード
    La_Card->Caption = Ini->ReadString("MainWnd", "La_Card", La_Card->Caption);//カード
    La_Title->Caption = Ini->ReadString("MainWnd", "La_Title", La_Title->Caption);//名前
    La_Size->Caption = Ini->ReadString("MainWnd", "La_Size", La_Size->Caption);//サイズ
    La_Label->Caption = Ini->ReadString("MainWnd", "La_Label", La_Label->Caption);//ラベル
    La_Link->Caption = Ini->ReadString("MainWnd", "La_Link", La_Link->Caption);//リンク
    La_LinkTitle->Caption = Ini->ReadString("MainWnd", "La_Title", La_LinkTitle->Caption);//名前
    Ch_LinkDirection->Caption = Ini->ReadString("MainWnd", "Ch_LinkDirection", Ch_LinkDirection->Caption);//向き
    Bu_LinkDirectionInverse->Caption = Ini->ReadString("MainWnd", "Bu_LinkDirectionInverse", Bu_LinkDirectionInverse->Caption);//反転
    La_LinkLabel->Caption = Ini->ReadString("MainWnd", "La_Label", La_LinkLabel->Caption);//ラベル
    La_Arrange->Caption = Ini->ReadString("MainWnd", "La_Arrange", La_Arrange->Caption);//自動整理
    La_Zoom->Caption = Ini->ReadString("MainWnd", "La_Zoom", La_Zoom->Caption);//ズーム
    Bu_Shuffle->Caption = Ini->ReadString("MainWnd", "Bu_Shuffle", Bu_Shuffle->Caption);//シャッフル
    SB_AutoScroll->Caption = Ini->ReadString("MainWnd", "SB_AutoScroll", SB_AutoScroll->Caption);//オートスクロール
    SB_AutoZoom->Caption = Ini->ReadString("MainWnd", "SB_AutoZoom", SB_AutoZoom->Caption);//オートズーム
    Bu_FindCard->Caption = Ini->ReadString("MainWnd", "Bu_FindCard", Bu_FindCard->Caption);//検索
    SB_EditorRelated->Caption = Ini->ReadString("MainWnd", "SB_EditorRelated", SB_EditorRelated->Caption);//関連テキスト
    La_PenColor->Caption = Ini->ReadString("MainWnd", "La_PenColor", La_PenColor->Caption);//色:
    La_PenColorDefault->Caption = Ini->ReadString("MainWnd", "La_PenColorDefault", La_PenColorDefault->Caption);//自動
    La_DZoom->Caption = Ini->ReadString("MainWnd", "La_Zoom", La_DZoom->Caption);//ズーム
    La_StatisticsKey->Caption = Ini->ReadString("MainWnd", "La_StatisticsKey", La_StatisticsKey->Caption);//キー
    SB_StatisticsSort->Caption = Ini->ReadString("MainWnd", "SB_StatisticsSort", SB_StatisticsSort->Caption);//ソート
    La_GlobalSearchTop->Caption = Ini->ReadString("MainWnd", "La_GlobalSearchTop", La_GlobalSearchTop->Caption);

    //メインメニュー
    M_File->Caption = Ini->ReadString("MainWnd", "M_File", M_File->Caption);//ファイル(&F)
    MF_New->Caption = Ini->ReadString("MainWnd", "MF_New", MF_New->Caption);
    MF_Open->Caption = Ini->ReadString("MainWnd", "MF_Open", MF_Open->Caption);//開く(&O)...
    MF_Save->Caption = Ini->ReadString("MainWnd", "MF_Save", MF_Save->Caption);//上書き保存(&S)
    MF_SaveAs->Caption = Ini->ReadString("MainWnd", "MF_SaveAs", MF_SaveAs->Caption);//名前をつけて保存(&A)...
    MF_RecentFiles->Caption = Ini->ReadString("MainWnd", "MF_RecentFiles", MF_RecentFiles->Caption);//最近使ったファイル(&R)
    MF_RecentFolders->Caption = Ini->ReadString("MainWnd", "MF_RecentFolders", MF_RecentFolders->Caption);//最近使ったフォルダ(&C)
    MF_Import->Caption = Ini->ReadString("MainWnd", "MF_Import", MF_Import->Caption);//インポート(&I)
    MFI_TxtFile->Caption = Ini->ReadString("MainWnd", "MFI_TxtFile", MFI_TxtFile->Caption);//テキストファイル(&T)...
    MFI_HierarchicalTextFile->Caption = Ini->ReadString("MainWnd", "MFI_HierarchicalTextFile", MFI_HierarchicalTextFile->Caption);
    MFI_HierarchicalTextFile2->Caption = Ini->ReadString("MainWnd", "MFI_HierarchicalTextFile2", MFI_HierarchicalTextFile2->Caption);
    MFI_TxtFilesinaFolder->Caption = Ini->ReadString("MainWnd", "MFI_TxtFilesinaFolder", MFI_TxtFilesinaFolder->Caption);
    MF_Export->Caption = Ini->ReadString("MainWnd", "MF_Export", MF_Export->Caption);//エクスポート(&E)
    MFE_TxtFileCardTitle->Caption = Ini->ReadString("MainWnd", "MFE_TxtFileCardTitle", MFE_TxtFileCardTitle->Caption);//テキストファイル（カードタイトル）(&C)...
    MFE_TxtFile->Caption = Ini->ReadString("MainWnd", "MFE_TxtFile", MFE_TxtFile->Caption);//テキストファイル(&T)...
    MFE_TxtFiles->Caption = Ini->ReadString("MainWnd", "MFE_TxtFiles", MFE_TxtFiles->Caption);//複数のテキストファイル(&X)...
    MFE_HierarchicalTextFile->Caption = Ini->ReadString("MainWnd", "MFE_HierarchicalTextFile", MFE_HierarchicalTextFile->Caption);
    MFE_HtmlFiles->Caption = Ini->ReadString("MainWnd", "MFE_HtmlFiles", MFE_HtmlFiles->Caption);//Htmlファイル(&H)...
    MFE_BMPFile->Caption = Ini->ReadString("MainWnd", "MFE_BMPFile", MFE_BMPFile->Caption);
    MFE_JPEGFile->Caption = Ini->ReadString("MainWnd", "MFE_JPEGFile", MFE_JPEGFile->Caption);
    MFE_ClipboardCardTitle->Caption = Ini->ReadString("MainWnd", "MFE_ClipboardCardTitle", MFE_ClipboardCardTitle->Caption);
    MFE_ClipboardCardText->Caption = Ini->ReadString("MainWnd", "MFE_ClipboardCardText", MFE_ClipboardCardText->Caption);
    MFE_ClipboardBMP->Caption = Ini->ReadString("MainWnd", "MFE_ClipboardBMP", MFE_ClipboardBMP->Caption);
    MF_PrintImage->Caption = Ini->ReadString("MainWnd", "MF_PrintImage", MF_PrintImage->Caption);//ブラウザ表示を印刷(&P)...
    MF_Exit->Caption = Ini->ReadString("MainWnd", "MF_Exit", MF_Exit->Caption);//終了(&X)
    M_Edit->Caption = Ini->ReadString("MainWnd", "M_Edit", M_Edit->Caption);//編集(&E)
    ME_Undo->Caption = Ini->ReadString("MainWnd", "ME_Undo", ME_Undo->Caption);//元に戻す(&U)
    ME_Redo->Caption = Ini->ReadString("MainWnd", "ME_Redo", ME_Redo->Caption);
    ME_Cut->Caption = Ini->ReadString("MainWnd", "ME_Cut", ME_Cut->Caption);//切り取り(&T)
    ME_Copy->Caption = Ini->ReadString("MainWnd", "ME_Copy", ME_Copy->Caption);//コピー(&C)
    ME_Paste->Caption = Ini->ReadString("MainWnd", "ME_Paste", ME_Paste->Caption);//貼り付け(&P)
    ME_Delete->Caption = Ini->ReadString("MainWnd", "ME_Delete", ME_Delete->Caption);//削除(&D)
    ME_SelectAll->Caption = Ini->ReadString("MainWnd", "ME_SelectAll", ME_SelectAll->Caption);//全て選択(&A)
    ME_Find->Caption = Ini->ReadString("MainWnd", "ME_Find", ME_Find->Caption);//検索(&F)
    ME_Replace->Caption = Ini->ReadString("MainWnd", "ME_Replace", ME_Replace->Caption);//(&E)
    ME_FindNext->Caption = Ini->ReadString("MainWnd", "ME_FindNext", ME_FindNext->Caption);//次を検索(&N)
    ME_FindPrevious->Caption = Ini->ReadString("MainWnd", "ME_FindPrevious", ME_FindPrevious->Caption);//前を検索(&V)
    ME_WebSearch->Caption = Ini->ReadString("MainWnd", "ME_WebSearch", ME_WebSearch->Caption);//W
    MD_SortCard->Caption = Ini->ReadString("MainWnd", "MD_SortCard", MD_SortCard->Caption);//カードの並び替え(&S)
    MDS_Title->Caption = Ini->ReadString("MainWnd", "MDS_Title", MDS_Title->Caption);//タイトル(&T)
    MDS_Title_I->Caption = Ini->ReadString("MainWnd", "MDS_Title_I", MDS_Title_I->Caption);//タイトル（降順）(&T)
    MDS_Date_Created->Caption = Ini->ReadString("MainWnd", "MDS_Date_Created", MDS_Date_Created->Caption);//作成日時(&C)
    MDS_Date_Created_I->Caption = Ini->ReadString("MainWnd", "MDS_Date_Created_I", MDS_Date_Created_I->Caption);//作成日時（降順）(&R)
    MDS_Date_Edited->Caption = Ini->ReadString("MainWnd", "MDS_Date_Edited", MDS_Date_Edited->Caption);//編集日時(&E)
    MDS_Date_Edited_I->Caption = Ini->ReadString("MainWnd", "MDS_Date_Edited_I", MDS_Date_Edited_I->Caption);//編集日時（降順）(&D)
    MDS_Date_Viewed->Caption = Ini->ReadString("MainWnd", "MDS_Date_Viewed", MDS_Date_Viewed->Caption);//閲覧日時(&V)
    MDS_Date_Viewed_I->Caption = Ini->ReadString("MainWnd", "MDS_Date_Viewed_I", MDS_Date_Viewed_I->Caption);//閲覧日時（降順）(&W)
    MDS_Score->Caption = Ini->ReadString("MainWnd", "MDS_Score", MDS_Score->Caption);//スコア(&S)
    MDS_Score_I->Caption = Ini->ReadString("MainWnd", "MDS_Score_I", MDS_Score_I->Caption);//スコア（降順）(&O)
    MDS_Shuffle->Caption = Ini->ReadString("MainWnd", "MDS_Shuffle", MDS_Shuffle->Caption);//シャッフル(&H)
    ME_BatchConversion->Caption = Ini->ReadString("MainWnd", "ME_BatchConversion", ME_BatchConversion->Caption);//一括変更(&B)
    MEC_AllCards->Caption = Ini->ReadString("MainWnd", "MEC_AllCards", MEC_AllCards->Caption);//全てのカードの形(&C)
    MEC_AllLinks->Caption = Ini->ReadString("MainWnd", "MEC_AllLinks", MEC_AllLinks->Caption);//全てのリンクの形(&L)
    MEC_AllLinksDirection->Caption = Ini->ReadString("MainWnd", "MEC_AllLinksDirection", MEC_AllLinksDirection->Caption);//全てのリンクの向き(&D)
    ME_Label->Caption = Ini->ReadString("MainWnd", "ME_Label", ME_Label->Caption);//ラベルの編集(&L)...
    ME_LinkLabel->Caption = Ini->ReadString("MainWnd", "ME_LinkLabel", ME_LinkLabel->Caption);//リンクラベルの編集(&I)...
    M_Insert->Caption = Ini->ReadString("MainWnd", "M_Insert", M_Insert->Caption);//挿入(&I)
    MI_NewCard->Caption = Ini->ReadString("MainWnd", "MI_NewCard", MI_NewCard->Caption);//新規カード(&C)
    MI_NewLink->Caption = Ini->ReadString("MainWnd", "MI_NewLink", MI_NewLink->Caption);//新規リンク(&L)...
    MI_NewExtLink->Caption = Ini->ReadString("MainWnd", "MI_NewExtLink", MI_NewExtLink->Caption);//新規外部リンク(&E)...
    MI_NewLabel->Caption = Ini->ReadString("MainWnd", "MI_NewLabel", MI_NewLabel->Caption);//新規ラベル(&A)...
    MI_NewLinkLabel->Caption = Ini->ReadString("MainWnd", "MI_NewLinkLabel", MI_NewLinkLabel->Caption);//新規リンクラベル(&N)...
    MI_LinktoAllCardswithDesignatedLabel->Caption = Ini->ReadString("MainWnd", "MI_LinktoAllCardswithDesignatedLabel", MI_LinktoAllCardswithDesignatedLabel->Caption);
    MI_NewCardLinkstoAllCardswithDesignatedLabel->Caption = Ini->ReadString("MainWnd", "MI_NewCardLinkstoAllCardswithDesignatedLabel", MI_NewCardLinkstoAllCardswithDesignatedLabel->Caption);
    MI_AddDesignatedLabeltoAllDestinationCards->Caption = Ini->ReadString("MainWnd", "MI_AddDesignatedLabeltoAllDestinationCards", MI_AddDesignatedLabeltoAllDestinationCards->Caption);
    M_Animation->Caption = Ini->ReadString("MainWnd", "M_Animation", M_Animation->Caption);//アニメーション(&A)
    MA_RandomFlash->Caption = Ini->ReadString("MainWnd", "MA_RandomFlash", MA_RandomFlash->Caption);//ランダムカード(&C)
    MA_RandomScroll->Caption = Ini->ReadString("MainWnd", "MA_RandomScroll", MA_RandomScroll->Caption);//ランダムカード2(&2)
    MA_RandomJump->Caption = Ini->ReadString("MainWnd", "MA_RandomJump", MA_RandomJump->Caption);//ランダムカード(&C)
    MA_RandomMap->Caption = Ini->ReadString("MainWnd", "MA_RandomMap", MA_RandomMap->Caption);//ランダムカード2(&2)
    MA_RandomTrace->Caption = Ini->ReadString("MainWnd", "MA_RandomTrace", MA_RandomTrace->Caption);//ランダムトレース(&T)
    M_View->Caption = Ini->ReadString("MainWnd", "M_View", M_View->Caption);//表示(&V)
    MV_Font->Caption = Ini->ReadString("MainWnd", "MV_Font", MV_Font->Caption);//フォント(&F)
    MVF_DefaultSize->Caption = Ini->ReadString("MainWnd", "MVF_DefaultSize", MVF_DefaultSize->Caption);//標準サイズ(&D)
    MVF_Magnify->Caption = Ini->ReadString("MainWnd", "MVF_Magnify", MVF_Magnify->Caption);//拡大(&M)
    MVF_Reduce->Caption = Ini->ReadString("MainWnd", "MVF_Reduce", MVF_Reduce->Caption);//縮小(&R)
    MVF_ChangeFont->Caption = Ini->ReadString("MainWnd", "MVF_ChangeFont", MVF_ChangeFont->Caption);//フォントの変更(&C)
    MV_Card->Caption = Ini->ReadString("MainWnd", "MV_Card", MV_Card->Caption);//カード(&C)
    MVC_CardShadow->Caption = Ini->ReadString("MainWnd", "MVC_CardShadow", MVC_CardShadow->Caption);
    MVC_CardGradation->Caption = Ini->ReadString("MainWnd", "MVC_CardGradation", MVC_CardGradation->Caption);
    MVC_TickerVisible->Caption = Ini->ReadString("MainWnd", "MVC_TickerVisible", MVC_TickerVisible->Caption);
    MVC_Ticker1Line->Caption = Ini->ReadString("MainWnd", "MVC_Ticker1Line", MVC_Ticker1Line->Caption);
    MVC_Ticker2Lines->Caption = Ini->ReadString("MainWnd", "MVC_Ticker2Lines", MVC_Ticker2Lines->Caption);
    MVC_Image->Caption = Ini->ReadString("MainWnd", "MVC_Image", MVC_Image->Caption);//イメージ(&I)
    MVC_Video->Caption = Ini->ReadString("MainWnd", "MVC_Video", MVC_Video->Caption);//ビデオ(&V)
    MVC_Drawing->Caption = Ini->ReadString("MainWnd", "MVC_Drawing", MVC_Drawing->Caption);//図(&D)
    MV_Link->Caption = Ini->ReadString("MainWnd", "MV_Link", MV_Link->Caption);//リンク(&L)
    MVL_Link->Caption = Ini->ReadString("MainWnd", "MVL_Link", MVL_Link->Caption);//リンク線(&L)
    MVL_LinkHemming->Caption = Ini->ReadString("MainWnd", "MVL_LinkHemming", MVL_LinkHemming->Caption);
    MVL_LinkShadow->Caption = Ini->ReadString("MainWnd", "MVL_LinkShadow", MVL_LinkShadow->Caption);
    MVL_LinkDirection->Caption = Ini->ReadString("MainWnd", "MVL_LinkDirection", MVL_LinkDirection->Caption);//リンクの方向(&D)
    MVL_LinkName->Caption = Ini->ReadString("MainWnd", "MVL_LinkName", MVL_LinkName->Caption);//リンク名(&N)
    MV_Label->Caption = Ini->ReadString("MainWnd", "MV_Label", MV_Label->Caption);//ラベル(&A)
    MVL_LabelCircle->Caption = Ini->ReadString("MainWnd", "MVL_LabelCircle", MVL_LabelCircle->Caption);//ラベルの円(&C)
    MVL_LabelRectangle->Caption = Ini->ReadString("MainWnd", "MVL_LabelRectangle", MVL_LabelRectangle->Caption);//ラベルの矩形(&R)
    MVL_LabelName->Caption = Ini->ReadString("MainWnd", "MVL_LabelName", MVL_LabelName->Caption);//ラベルの円(&N)
    MV_Text->Caption = Ini->ReadString("MainWnd", "MV_Text", MV_Text->Caption);//本文テキスト(&T)
    MVT_Text->Caption = Ini->ReadString("MainWnd", "MVT_Text", MVT_Text->Caption);//本文テキスト(&T)
    MVT_Centering->Caption = Ini->ReadString("MainWnd", "MVT_Centering", MVT_Centering->Caption);//センタリング(&C)
    MVT_WordWrap->Caption = Ini->ReadString("MainWnd", "MVT_WordWrap", MVT_WordWrap->Caption);//右端で折り返す(&W)
    MVT_EditInBrowser->Caption = Ini->ReadString("MainWnd", "MVT_EditInBrowser", MVT_EditInBrowser->Caption);
    MVT_AlwaysShowEditor->Caption = Ini->ReadString("MainWnd", "MVT_AlwaysShowEditor", MVT_AlwaysShowEditor->Caption);
    MVT_UnderTheCard->Caption = Ini->ReadString("MainWnd", "MVT_UnderTheCard", MVT_UnderTheCard->Caption);
    MVT_BrowserRight->Caption = Ini->ReadString("MainWnd", "MVT_BrowserRight", MVT_BrowserRight->Caption);
    MVT_BrowserBottom->Caption = Ini->ReadString("MainWnd", "MVT_BrowserBottom", MVT_BrowserBottom->Caption);
    MV_Others->Caption = Ini->ReadString("MainWnd", "MV_Others", MV_Others->Caption);//その他(&O)
    MV_Score->Caption = Ini->ReadString("MainWnd", "MV_Score", MV_Score->Caption);//スコア(&S)
    MVS_Score->Caption = Ini->ReadString("MainWnd", "MVS_Score", MVS_Score->Caption);//スコア(&S)
    MVS_Authenticity->Caption = Ini->ReadString("MainWnd", "MVS_Authenticity", MVS_Authenticity->Caption);//信頼度(&A)
    MVS_StartingPoint->Caption = Ini->ReadString("MainWnd", "MVS_StartingPoint", MVS_StartingPoint->Caption);//開始地点(&S)
    MVS_Destination->Caption = Ini->ReadString("MainWnd", "MVS_Destination", MVS_Destination->Caption);//目的地(&D)
    MVS_Links_Out->Caption = Ini->ReadString("MainWnd", "MVS_Links_Out", MVS_Links_Out->Caption);//リンク数（出力）(&O)
    MVS_Links_In->Caption = Ini->ReadString("MainWnd", "MVS_Links_In", MVS_Links_In->Caption);//リンク数（入力）(&I)
    MVS_Links_Total->Caption = Ini->ReadString("MainWnd", "MVS_Links_Total", MVS_Links_Total->Caption);//リンク数（合計）(&T)
    MVS_Links_InOut->Caption = Ini->ReadString("MainWnd", "MVS_Links_InOut", MVS_Links_InOut->Caption);//リンク数（入力-出力）(&L)
    MVS_TextLength->Caption = Ini->ReadString("MainWnd", "MVS_TextLength", MVS_TextLength->Caption);//本文の長さ(&X)
    MV_Read->Caption = Ini->ReadString("MainWnd", "MV_Read", MV_Read->Caption);//読み上げ(&R)
    MVR_Read->Caption = Ini->ReadString("MainWnd", "MVR_Read", MVR_Read->Caption);//読み上げ(&R)
    MVR_ReadSetting->Caption = Ini->ReadString("MainWnd", "MVR_ReadSetting", MVR_ReadSetting->Caption);//読み上げの設定(&E)
    MVO_CardList->Caption = Ini->ReadString("MainWnd", "MVO_CardList", MVO_CardList->Caption);//
    MVO_FileList->Caption = Ini->ReadString("MainWnd", "MVO_FileList", MVO_FileList->Caption);//
    MVO_Overview->Caption = Ini->ReadString("MainWnd", "MVO_Overview", MVO_Overview->Caption);//全体像を表示(&O)
    MVO_ChangeFourgroundColor->Caption = Ini->ReadString("MainWnd", "MVO_ChangeFourgroundColor", MVO_ChangeFourgroundColor->Caption);//前景色を変更(&F)...
    MVO_ChangeBackgroundColor->Caption = Ini->ReadString("MainWnd", "MVO_ChangeBackgroundColor", MVO_ChangeBackgroundColor->Caption);//背景色を変更(&B)...
    MVO_WallPaper->Caption = Ini->ReadString("MainWnd", "MVO_WallPaper", MVO_WallPaper->Caption);//壁紙(&W)...
    MVO_FixWallPaper->Caption = Ini->ReadString("MainWnd", "MVO_FixWallPaper", MVO_FixWallPaper->Caption);//壁紙を固定(&I)
    MVO_TileWallPaper->Caption = Ini->ReadString("MainWnd", "MVO_TileWallPaper", MVO_TileWallPaper->Caption);//壁紙を並べて表示(&T)
    MVO_AntiAliasing->Caption = Ini->ReadString("MainWnd", "MVO_AntiAliasing", MVO_AntiAliasing->Caption);//アンチエイリアス(&A)
    MVO_BGAnimation->Caption = Ini->ReadString("MainWnd", "MVO_BGAnimation", MVO_BGAnimation->Caption);//&C
    MVO_BGAnimationType->Caption = Ini->ReadString("MainWnd", "MVO_BGAnimationType", MVO_BGAnimationType->Caption);//&K
    MVO_NoScrollLag->Caption = Ini->ReadString("MainWnd", "MVO_NoScrollLag", MVO_NoScrollLag->Caption);//&N
    MVO_CursorAnimation->Caption = Ini->ReadString("MainWnd", "MVO_CursorAnimation", MVO_CursorAnimation->Caption);//&U
    MVOB_MovingLine->Caption = Ini->ReadString("MainWnd", "MVOB_MovingLine", MVOB_MovingLine->Caption);//&M
    MVOB_Bubble->Caption = Ini->ReadString("MainWnd", "MVOB_Bubble", MVOB_Bubble->Caption);//&B
    MVOB_Snow->Caption = Ini->ReadString("MainWnd", "MVOB_Snow", MVOB_Snow->Caption);//&S
    MVOB_CherryBlossom->Caption = Ini->ReadString("MainWnd", "MVOB_CherryBlossom", MVOB_CherryBlossom->Caption);//&C
    MV_ChangeLanguage->Caption = Ini->ReadString("MainWnd", "MV_ChangeLanguage", MV_ChangeLanguage->Caption);//言語の変更(&H)
    MV_FullScreen->Caption = Ini->ReadString("MainWnd", "MV_FullScreen", MV_FullScreen->Caption);//フルスクリーン表示(&U)
    MV_StatusBar->Caption = Ini->ReadString("MainWnd", "MV_StatusBar", MV_StatusBar->Caption);//ステータスバー(&B)
    M_Help->Caption = Ini->ReadString("MainWnd", "M_Help", M_Help->Caption);//ヘルプ(&H)
    MH_Contents->Caption = Ini->ReadString("MainWnd", "MH_Contents", MH_Contents->Caption);//目次(&C)...
    MH_FrieveSite->Caption = Ini->ReadString("MainWnd", "MH_FrieveSite", MH_FrieveSite->Caption);//Frieveのサイト(&F)...
    MH_FIPSite->Caption = Ini->ReadString("MainWnd", "MH_FIPSite", MH_FIPSite->Caption);//Frieve Editorのサイト(&O)...
    MH_CheckLatest->Caption = Ini->ReadString("MainWnd", "MH_CheckLatest", MH_CheckLatest->Caption);//最新バージョンを確認(&L)...
    MH_About->Caption = Ini->ReadString("MainWnd", "MH_About", MH_About->Caption);//バージョン情報(&A)...

    //ポップアップメニュー（エディタ）
    PE_Undo->Caption = Ini->ReadString("MainWnd", "ME_Undo", PE_Undo->Caption);//元に戻す(&U)
    PE_Redo->Caption = Ini->ReadString("MainWnd", "ME_Redo", PE_Redo->Caption);
    PE_Cut->Caption = Ini->ReadString("MainWnd", "ME_Cut", PE_Cut->Caption);//切り取り(&T)
    PE_Copy->Caption = Ini->ReadString("MainWnd", "ME_Copy", PE_Copy->Caption);//コピー(&C)
    PE_Paste->Caption = Ini->ReadString("MainWnd", "ME_Paste", PE_Paste->Caption);//貼り付け(&P)
    PE_Delete->Caption = Ini->ReadString("MainWnd", "ME_Delete", PE_Delete->Caption);//削除(&D)
    PE_SelectAll->Caption = Ini->ReadString("MainWnd", "ME_SelectAll", PE_SelectAll->Caption);//全て選択(&A)
    PE_CutToNewCard->Caption = Ini->ReadString("MainWnd", "PM_CutToNewCard", PE_CutToNewCard->Caption);
    PE_CutToNewCardTitle->Caption = Ini->ReadString("MainWnd", "PM_CutToNewCardTitle", PE_CutToNewCardTitle->Caption);
    PE_CutToNewCardTitleWithLink->Caption = Ini->ReadString("MainWnd", "PM_CutToNewCardTitleWithLink", PE_CutToNewCardTitleWithLink->Caption);

    //ポップアップメニュー（リスト）
    PL_NewCard->Caption = Ini->ReadString("MainWnd", "MI_NewCard", PL_NewCard->Caption);//新規カード(&C)
    PL_DeleteCard->Caption = Ini->ReadString("MainWnd", "PM_DeleteCard", PL_DeleteCard->Caption);
    PL_CardProperty->Caption = Ini->ReadString("MainWnd", "PM_Property", PL_CardProperty->Caption);

    PI_NewLink->Caption = Ini->ReadString("MainWnd", "MI_NewLink", PI_NewLink->Caption);
    PI_DeleteLink->Caption = Ini->ReadString("MainWnd", "PM_DeleteLink", PI_DeleteLink->Caption);

    //ポップアップメニュー（ブラウザ）
    PBN_Select->Caption = Ini->ReadString("MainWnd", "PBN_Select", PBN_Select->Caption);
    PBN_Link->Caption = Ini->ReadString("MainWnd", "PBN_Link", PBN_Link->Caption);
    PBN_Undo->Caption = Ini->ReadString("MainWnd", "ME_Undo", PBN_Undo->Caption);//元に戻す(&U)
    PBN_Redo->Caption = Ini->ReadString("MainWnd", "ME_Redo", PBN_Redo->Caption);
    PBN_Paste->Caption = Ini->ReadString("MainWnd", "ME_Paste", PBN_Paste->Caption);//貼り付け(&P)
    PBN_NewCard->Caption = Ini->ReadString("MainWnd", "MI_NewCard", PBN_NewCard->Caption);//新規カード(&C)

    PBC_Select->Caption = Ini->ReadString("MainWnd", "PBN_Select", PBC_Select->Caption);
    PBC_Link->Caption = Ini->ReadString("MainWnd", "PBN_Link", PBC_Link->Caption);
    PBC_Undo->Caption = Ini->ReadString("MainWnd", "ME_Undo", PBN_Undo->Caption);//元に戻す(&U)
    PBC_Redo->Caption = Ini->ReadString("MainWnd", "ME_Redo", PBN_Redo->Caption);
    PBC_Cut->Caption = Ini->ReadString("MainWnd", "ME_Cut", PBC_Cut->Caption);//切り取り(&T)
    PBC_Copy->Caption = Ini->ReadString("MainWnd", "ME_Copy", PBC_Copy->Caption);//コピー(&C)
    PBC_Paste->Caption = Ini->ReadString("MainWnd", "ME_Paste", PBC_Paste->Caption);//貼り付け(&P)
    PBC_WebSearch->Caption = Ini->ReadString("MainWnd", "ME_WebSearch", ME_WebSearch->Caption);//W
    PBC_NewCard->Caption = Ini->ReadString("MainWnd", "MI_NewCard", PBC_NewCard->Caption);//新規カード(&C)
    PBC_NewChildCard->Caption = Ini->ReadString("MainWnd", "PM_NewChildCard", PBC_NewChildCard->Caption);
    PBC_NewBrotherCard->Caption = Ini->ReadString("MainWnd", "PM_NewBrotherCard", PBC_NewBrotherCard->Caption);
    PBC_NewLink->Caption = Ini->ReadString("MainWnd", "MI_NewLink", PBC_NewLink->Caption);
    PBC_NewExtLink->Caption = Ini->ReadString("MainWnd", "MI_NewExtLink", PBC_NewExtLink->Caption);
    PBC_NewLabel->Caption = Ini->ReadString("MainWnd", "MI_NewLabel", PBC_NewLabel->Caption);
    PBC_DeleteCard->Caption = Ini->ReadString("MainWnd", "PM_DeleteCard", PBC_DeleteCard->Caption);
    PBC_SetAsDefault->Caption = Ini->ReadString("MainWnd", "PM_SetAsDefault", PBC_SetAsDefault->Caption);
    PBC_Property->Caption = Ini->ReadString("MainWnd", "PM_Property", PBC_Property->Caption);

    PBL_Select->Caption = Ini->ReadString("MainWnd", "PBN_Select", PBL_Select->Caption);
    PBL_Link->Caption = Ini->ReadString("MainWnd", "PBN_Link", PBL_Link->Caption);
    PBL_Undo->Caption = Ini->ReadString("MainWnd", "ME_Undo", PBL_Undo->Caption);//元に戻す(&U)
    PBL_Redo->Caption = Ini->ReadString("MainWnd", "ME_Redo", PBL_Redo->Caption);
    PBL_WebSearch->Caption = Ini->ReadString("MainWnd", "ME_WebSearch", ME_WebSearch->Caption);//W
    PBL_NewCard->Caption = Ini->ReadString("MainWnd", "MI_NewCard", PBL_NewCard->Caption);//新規カード(&C)
    PBL_InsertCard->Caption = Ini->ReadString("MainWnd", "PM_InsertCard", PBL_InsertCard->Caption);
    PBL_NewLinkLabel->Caption = Ini->ReadString("MainWnd", "MI_NewLinkLabel", PBL_NewLinkLabel->Caption);
    PBL_DeleteLink->Caption = Ini->ReadString("MainWnd", "PM_DeleteLink", PBL_DeleteLink->Caption);
    PBL_SetAsDefault->Caption = Ini->ReadString("MainWnd", "PM_SetAsDefault", PBL_SetAsDefault->Caption);

    //ポップアップメニュー（アレンジ）
    PAT_Normalize->Caption = Ini->ReadString("MainWnd", "PAT_Normalize", PAT_Normalize->Caption);//ノーマライズ(&N)
    PAT_Repulsion->Caption = Ini->ReadString("MainWnd", "PAT_Repulsion", PAT_Repulsion->Caption);//反発(&R)
    PAT_Link->Caption = Ini->ReadString("MainWnd", "PAT_Link", PAT_Link->Caption);//リンク(&N)
    PAT_Label->Caption = Ini->ReadString("MainWnd", "PAT_Label", PAT_Label->Caption);//ラベル(&A)
    PAT_Index->Caption = Ini->ReadString("MainWnd", "PAT_Index", PAT_Index->Caption);//順序(&I)
    PAT_SoftLink->Caption = Ini->ReadString("MainWnd", "PAT_SoftLink", PAT_SoftLink->Caption);//リンク(ソフト)(&K)
    PAT_SoftLabel->Caption = Ini->ReadString("MainWnd", "PAT_SoftLabel", PAT_SoftLabel->Caption);//ラベル(ソフト)(&B)
    PAT_SoftIndex->Caption = Ini->ReadString("MainWnd", "PAT_SoftIndex", PAT_SoftIndex->Caption);//順序(ソフト)(&D)
    PAT_Matrix->Caption = Ini->ReadString("MainWnd", "PAT_Matrix", PAT_Matrix->Caption);//マトリクス(&M)
    PAT_MatrixIndex->Caption = Ini->ReadString("MainWnd", "PAT_MatrixIndex", PAT_MatrixIndex->Caption);//順序(マトリクス)(&E)
    PAT_Similarity->Caption = Ini->ReadString("MainWnd", "PAT_Similarity", PAT_Similarity->Caption);//類似度(&S)
    PAT_SimilaritySoft->Caption = Ini->ReadString("MainWnd", "PAT_SimilaritySoft", PAT_SimilaritySoft->Caption);//類似度(ソフト)
    PAT_TreeRadial->Caption = Ini->ReadString("MainWnd", "PAT_TreeRadial", PAT_TreeRadial->Caption);//ツリー(放射状)(&T)

    //ポップアップメニュー（統計）
    PMSK_Label->Caption = Ini->ReadString("MainWnd", "PMSK_Label", PMSK_Label->Caption);//ラベル(&L)
    PMSK_NumberOfLink->Caption = Ini->ReadString("MainWnd", "PMSK_NumberOfLink", PMSK_NumberOfLink->Caption);//リンク数(&N)
    PMSK_CreatedDate->Caption = Ini->ReadString("MainWnd", "PMSK_CreatedDate", PMSK_CreatedDate->Caption);//作成日時(&C)
    PMSK_EditedDate->Caption = Ini->ReadString("MainWnd", "PMSK_EditedDate", PMSK_EditedDate->Caption);//編集日時(&E)
    PMSK_ViewedDate->Caption = Ini->ReadString("MainWnd", "PMSK_ViewedDate", PMSK_ViewedDate->Caption);//閲覧日時(&V)
    PMSKL_Total->Caption = Ini->ReadString("MainWnd", "PMSKL_Total", PMSKL_Total->Caption);//合計(&T)
    PMSKL_Source->Caption = Ini->ReadString("MainWnd", "PMSKL_Source", PMSKL_Source->Caption);//リンク元(&S)
    PMSKL_Destination->Caption = Ini->ReadString("MainWnd", "PMSKL_Destination", PMSKL_Destination->Caption);//リンク先(&D)
    PMSKC_Year->Caption = Ini->ReadString("MainWnd", "PMSK_Year", PMSKC_Year->Caption);//年毎に集計(&Y)
    PMSKC_Month->Caption = Ini->ReadString("MainWnd", "PMSK_Month", PMSKC_Month->Caption);//月毎に集計(&M)
    PMSKC_Day->Caption = Ini->ReadString("MainWnd", "PMSK_Day", PMSKC_Day->Caption);//日毎に集計(&D)
    PMSKC_Week->Caption = Ini->ReadString("MainWnd", "PMSK_Week", PMSKC_Week->Caption);//週毎に集計(&W)
    PMSKC_Hour->Caption = Ini->ReadString("MainWnd", "PMSK_Hour", PMSKC_Hour->Caption);//時間毎に集計(&H)
    PMSKE_Year->Caption = Ini->ReadString("MainWnd", "PMSK_Year", PMSKE_Year->Caption);//年毎に集計(&Y)
    PMSKE_Month->Caption = Ini->ReadString("MainWnd", "PMSK_Month", PMSKE_Month->Caption);//月毎に集計(&M)
    PMSKE_Day->Caption = Ini->ReadString("MainWnd", "PMSK_Day", PMSKE_Day->Caption);//日毎に集計(&D)
    PMSKE_Week->Caption = Ini->ReadString("MainWnd", "PMSK_Week", PMSKE_Week->Caption);//週毎に集計(&W)
    PMSKE_Hour->Caption = Ini->ReadString("MainWnd", "PMSK_Hour", PMSKE_Hour->Caption);//時間毎に集計(&H)
    PMSKV_Year->Caption = Ini->ReadString("MainWnd", "PMSK_Year", PMSKV_Year->Caption);//年毎に集計(&Y)
    PMSKV_Month->Caption = Ini->ReadString("MainWnd", "PMSK_Month", PMSKV_Month->Caption);//月毎に集計(&M)
    PMSKV_Day->Caption = Ini->ReadString("MainWnd", "PMSK_Day", PMSKV_Day->Caption);//日毎に集計(&D)
    PMSKV_Week->Caption = Ini->ReadString("MainWnd", "PMSK_Week", PMSKV_Week->Caption);//週毎に集計(&W)
    PMSKV_Hour->Caption = Ini->ReadString("MainWnd", "PMSK_Hour", PMSKV_Hour->Caption);//時間毎に集計(&H)
    ...
    MLText.EditCardTitle = Ini->ReadString("Common", "EditCardTitle", MLText.EditCardTitle);
    MLText.EditCardText = Ini->ReadString("Common", "EditCardText", MLText.EditCardText);
    MLText.EditLinkTitle = Ini->ReadString("Common", "EditLinkTitle", MLText.EditLinkTitle);
    MLText.EditDrawing = Ini->ReadString("Common", "EditDrawing", MLText.EditDrawing);

    MLText.ChangeSize = Ini->ReadString("Common", "ChangeSize", MLText.ChangeSize);
    MLText.FixCardPosition = Ini->ReadString("Common", "FixCardPosition", MLText.FixCardPosition);
    MLText.SetAsRootCard = Ini->ReadString("Common", "SetAsRootCard", MLText.SetAsRootCard);
    ...
    MLText.DeleteLabel = Ini->ReadString("Common", "DeleteLabel", MLText.DeleteLabel);
    MLText.ChangeLabel = Ini->ReadString("Common", "ChangeLabel", MLText.ChangeLabel);
    MLText.LabelName = Ini->ReadString("Common", "LabelName", MLText.LabelName);
    MLText.SelectLinkDestination = Ini->ReadString("Common", "SelectLinkDestination", MLText.SelectLinkDestination);
    MLText.SelectLabelOfDestination = Ini->ReadString("Common", "SelectLabelOfDestination", MLText.SelectLabelOfDestination);
    MLText.SelectLabel = Ini->ReadString("Common", "SelectLabel", MLText.SelectLabel);

    MLText.Color = Ini->ReadString("Common", "Color", MLText.Color);

    MLText.ArrangeCards = Ini->ReadString("Common", "ArrangeCards", MLText.ArrangeCards);

    MLText.ProcessingTextAnalysis = Ini->ReadString("Common", "ProcessingTextAnalysis", MLText.ProcessingTextAnalysis);
    ...
    MLText.NoAssign  = Ini->ReadString("Common", "NoAssign", MLText.NoAssign);//未指定
    MLText.NoLabel  = Ini->ReadString("Common", "NoLabel", MLText.NoLabel);//ラベルなし
    MLText.SelectTargetCard  = Ini->ReadString("Common", "SelectTargetCard", MLText.SelectTargetCard);//リンク先カードの選択
    ...
    MLText.FailedToSave = Ini->ReadString("Common", "FailedToSave", MLText.FailedToSave);
    MLText.NotFound = Ini->ReadString("Common", "NotFound", MLText.NotFound);
    MLText.AANotEnoughMemory = Ini->ReadString("Common", "AANotEnoughMemory", MLText.AANotEnoughMemory);
    delete Ini;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
TCardImage *TFo_Main::SearchImage(AnsiString FN)
{
...
bool TFo_Main::UpdateImageList()
{
    ...
    //カードループ
    if (SettingView.m_bImageVisible) for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
        TCard *Card = m_Document->GetCardByIndex(i);
...
//---------------------------------------------------------------------------
TCardVideo *TFo_Main::SearchVideo(AnsiString FN)
{
...
bool TFo_Main::UpdateVideoList()
{
    ...
    //カードループ
    if (SettingView.m_bVideoVisible) for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
        TCard *Card = m_Document->GetCardByIndex(i);
...
void __fastcall TFo_Main::Bu_TestClick(TObject *Sender)
{
    ...
    float fdummy;
    bool bdummy = false;
    TTextDecomposer *TD = new TTextDecomposer(WS, 10, 100, 0.0f, 0.0f, fdummy, bdummy);
    RE_Edit->Clear();
    RE_Edit->Lines->BeginUpdate();
    RE_Edit->Lines->Add(AnsiString("Time elapsed : ") + IntToStr(timeGetTime() - t) + "ms");
    RE_Edit->Lines->Add(AnsiString("Items : ") + IntToStr(TD->m_nMaxSN));
    RE_Edit->Lines->Add("");
    for (int i = 0 ; i < TD->m_nMaxCombi ; i++){
        RE_Edit->Lines->Add(AnsiString("<<<<<  ") + IntToStr(i + 1) + "-gram  >>>>>");
        RE_Edit->Lines->Add("");
        WideString WS;
        for (int i2 = 0 ; i2 < TD->m_Gram[i]->Count ; i2++){
            if (TD->m_Gram[i]->Enabled(i2)){
                WS +=
                    TD->m_Gram[i]->Strings(i2) +
                    WideString("(") +
                    IntToStr(TD->m_Gram[i]->Values(i2)) +
                    WideString("),");
            }
    ...
    */
    ...
    SM->Finalize(true);
    RE_Edit->Lines->Add(AnsiString("SM : ") + IntToStr(timeGetTime() - t) + "ms  (Ex : " + IntToStr(t2 - t) + "ms  Fn : " + IntToStr(timeGetTime() - t2) + "ms)");
    RE_Edit->Lines->Add(AnsiString("SM Items : ") + SM->m_Items->Count);
    RE_Edit->Lines->Add("");
    ...
    RE_Edit->Lines->Add(AnsiString("SM Items (after DeleteSameCol): ") + SM->m_Items->Count);
    RE_Edit->Lines->Add(AnsiString("DM : ") + IntToStr(timeGetTime() - t) + "ms  (Cos : " + IntToStr(t2 - t) + "ms  Fn : " + IntToStr(timeGetTime() - t2) + "ms)");
...
void __fastcall TFo_Main::PAT_ArrangeTypeClick(TObject *Sender)
{
    ...
    if (SB_Arrange->Down && tree != (Bu_ArrangeType->Tag >= 1000 && Bu_ArrangeType->Tag <= 1999)){
        m_Document->RefreshList();
    }
    PrepareArrange();
}
...
void TFo_Main::RefreshSimMatrix_()
{
    ...
    TTextDecomposer *TD = new TTextDecomposer(WS, 10, 100, 0.0f, 0.6f, Fo_Progress->fPos, Fo_Progress->m_bTerminated);
...
//---------------------------------------------------------------------------
float TFo_Main::MoveDistance(float sx, float sy, float dx, float dy, int direction)
{
...
//---------------------------------------------------------------------------
void TFo_Main::ExportBMP(int operation, AnsiString Text)
{
...
//---------------------------------------------------------------------------
void TFo_Main::ExportCardText(int operation, bool btitle, AnsiString Text)
{
    TStringList *SL = new TStringList();
    ...
    case 0:
        try{
            ...
            ShowMessage(MLText.FailedToSave + SD_Txt->FileName + AnsiString("."));
        }
        break;
    case 1:
...
void __fastcall TFo_Main::MFI_HierarchicalTextFileClick(TObject *Sender)
{
    //階層テキスト読み込み
    if (OD_Txt->Execute()){
        BackupSub(MF_Import->Caption + AnsiString(" ") + MFI_HierarchicalTextFile->Caption);
        ...
        for (int i = 0 ; i < OD_Txt->Files->Count ; i++){
            ...
            //階層記号決定
            for (int il = 0 ; il < SL->Count ; il++) if (SL->Strings[il] != ""){
            ...
            //行ループ
            for (int il = 0 ; il < SL->Count ; il++) if (Trim(SL->Strings[il]) != ""){
                TCard *Card = m_Document->NewCard(m_Document->m_Cards->Count);
                int level = 1;
                WideString W = SL->Strings[il];
                while (true){
...
//---------------------------------------------------------------------------
/*
AnsiString SelectDirectory(AnsiString stTitle)
{
    HResult hres;
    int *ip0, ip1;
...
void TFo_Main::AddTxtsInFolder(AnsiString TopDir, TCard *Parent)
{
    TCard *Card = m_Document->NewCard(m_Document->m_Cards->Count);
...
void __fastcall TFo_Main::MFI_TxtFilesinaFolderClick(TObject *Sender)
{
    ...
    if (SelectDirectory("", "", Dir)){
        BackupSub(MF_Import->Caption + AnsiString(" ") + MFI_TxtFilesinaFolder->Caption);
        ...
        for (int i = 0 ; i < Files->Count ; i++){
            AnsiString Dir = ExtractFileDir(Files->Strings[i]);
            if (Dirs->Count == 0){
                AnsiString TopDir = ExtractFileDir(Dir);
                if (Dir == ""){
                    "No Title";
                }
                Dirs->Add(TopDir);
...
void __fastcall TFo_Main::Bu_BrowserTestClick(TObject *Sender)
{
    ...
    if (b){
        ...
        m_UndoRedo->Redo(m_Document, m_nCurrentCard, RE_Edit->SelStart, RE_Edit->SelLength, &m_nNextCardID, &m_nNextSelStart, &m_nNextSelLength);
    }
...
void __fastcall TFo_Main::LB_ListMouseDown(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if (Button == mbLeft){
        Ti_Check->Enabled = false;
...
void __fastcall TFo_Main::LB_ListMouseUp(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if (Button == mbLeft){
        Ti_Check->Enabled = true;
...
{
    if (PC_Client->ActivePage != TS_Editor && (!SettingView.m_bEditInBrowserAlways || Fo_FullScreen->Visible)){
        CloseTextEditBox();
    }
}
//---------------------------------------------------------------------------
...
{
    TStringList *SL = new TStringList();
    FileListCreator(ExtractFileDir(SettingFile.m_RecentFolders[0] + "file"), SL, ".fip", false);
    ...
    for (int i = 0 ; i < itemcount + 1 ; i++){
        ...
        if (Index < m_GlobalSearchResult->Count){
            ...
            if (Card){
                ...
                C->RoundRect(0, top + 2, BMP->Width - 3, top + m_GlobalSearchItemHeight - 2, m_GlobalSearchItemHeight / 4, m_GlobalSearchItemHeight / 4);

                HRGN MyRgn;
                MyRgn = ::CreateRectRgn(textheight / 2 - 2, 0, BMP->Width - textheight / 2, BMP->Height);
                ::SelectClipRgn(C->Handle, MyRgn);
                ...
                //カードタイトルを描画
                {
                    ...
                    if (foundat == 0){
                        ...
                        //右側を表示
                        S1 = Title.SubString(foundat + m_GlobalSearchKeyword.Length(), Title.Length());
                        C->TextOut(textheight / 2 - 2 + widthsum, top + textheight / 2, S1);
                    }
                }
                    ...
                    if (foundat == 0){
                        ...
                        */
                        while (C->TextWidth(Text) > BMP->Width - textheight / 2){
                            ...
                            int rightlen = Text.Length() - m_GlobalSearchKeyword.Length() - (foundat - 1);
                            int delpos;
                        ...
                        int widthsum = 0;
                        C->TextOut(textheight / 2 - 2, top + textheight + textheight / 2, S1);
                        widthsum += C->TextWidth(S1);
                        ...
                        F->Color = clRed;
                        F->Style = TFontStyles() << fsBold;
                        C->TextOut(textheight / 2 - 2 + widthsum, top + textheight + textheight / 2, S1);
                        widthsum += C->TextWidth(S1);
                        F->Color = TColor(m_nFGColor);
                        F->Style = TFontStyles();

                        //右側を表示
                        S1 = Text.SubString(foundat + m_GlobalSearchKeyword.Length(), Text.Length());
                        C->TextOut(textheight / 2 - 2 + widthsum, top + textheight + textheight / 2, S1);
                    }
                }
                ::SelectClipRgn(C->Handle, NULL);
...
void __fastcall TFo_Main::Sc_GlobalSearchKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
    ...
    if (targetindex != m_GlobalSearchCursorIndex){
        Key = 0;
        TCard *Card = m_Document->GetCard((int)m_GlobalSearchResult->Items[targetindex]);
        m_Document->ClearCardSelection();
        Card->m_bSelected = true;
        m_nTargetCard = Card->m_nID;
...
void __fastcall TFo_Main::PE_CutToNewCardTitleWithLinkClick(
      TObject *Sender)
{
...
void __fastcall TFo_Main::MFI_HierarchicalTextFile2Click(TObject *Sender)
{
    //階層テキスト読み込み
    if (OD_Txt->Execute()){
        BackupSub(MF_Import->Caption + AnsiString(" ") + MFI_HierarchicalTextFile2->Caption);
    ...
    if (SB_EditorRelated->Down && LB_Link->ItemIndex >= 0){
        ...
        if (Link){
            ...
            if (foundat >= 1){
                ...
                Me_EditorRelated->SelLength = Key.Length();//AnsiString(SearchKeyword).Length();
            }else{
...
void __fastcall TFo_Main::PB_StatisticsPaint(TObject *Sender)
{
...

    {
        ...
        case 40400:
            {
                ...
                case 2:
                    {
                        ...
                        for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
                            ...
                            double date = 0.0;
                            switch (type1){
                            case 2: date = Card->m_fCreated; break;
                            case 3: date = Card->m_fUpdated; break;
                            case 4: date = Card->m_fViewed; break;
                            }
                            dt[i] = date;
                        }
                        //全カードの日時をソート
                        ...
                        for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
                            AnsiString S;
                            switch (type2){
                            case 0: S = IntToStr(YearOf(dt[i])); break;
                            case 1: S = IntToStr(MonthOf(dt[i])) + "/" + IntToStr(YearOf(dt[i])); break;
                            case 2: S = DateToStr(dt[i]); break;
                            }
                ...
                for (int i = 0 ; i < m_Document->m_Cards->Count ; i++){
                    ...
                    case 2:
                        {
                            AnsiString S;
                            switch (type2){
                            case 0: S = IntToStr(YearOf(date)); break;
                            case 1: S = IntToStr(MonthOf(date)) + "/" + IntToStr(YearOf(date)); break;
                            case 2: S = DateToStr(date); break;
                            }
                    ...
                    TSRectToCard *SRTC = (TSRectToCard *)m_StatisticsRectToCard->Items[index];
                    SRTC->m_CardIDList->Add((void*)Card->m_nID);
                }
            }
            break;
            ...
            for (int i = 1 ; i < Value->Count ; i++){
                for (int i2 = i ; i2 > 0 ; i2--){
                    ...
                    if (v1 > v2){
                        ...
                        m_StatisticsRectToCard->Items[i2] = m_StatisticsRectToCard->Items[i2 - 1];
        ...
        float hvisible = (BMP->Height - topmargin - leftmargin) / lineheignt;//1画面にいくつバーが入るか
        if (hvisible * 2 >= Name->Count || Name->Count == 0){
            Sc_StatisticsY->LargeChange = 10000;
        }else{
            Sc_StatisticsY->LargeChange = (10000 * hvisible) / (Name->Count - hvisible);
        }
        Sc_StatisticsY->SmallChange = Sc_StatisticsY->LargeChange / 10;
        float hinvisible = Name->Count - hvisible;//画面に入りきらないバーの数
        int hoffset = hinvisible * Sc_StatisticsY->Position * 0.0001 * lineheignt;//現在のスクロール位置
        if (hoffset < 0){
            hoffset = 0;
        }
        ...
        for (int i = 0 ; i < Name->Count ; i++){
            ...
            if (y < BMP->Height - leftmargin && y + m_nFontHeight * 2 >= topmargin){
                ...
                SRTC->m_Rect = Rect(0, y - m_nFontHeight / 4, BMP->Width, y + m_nFontHeight * 1.5 + m_nFontHeight / 4);
                ...
                int l = (int)Value->Items[i];
                float v = *(float*)&l;
                int x = leftmargin + textwidth + leftmargin * 2 + barwidth * v / max * m_fStatisticsPos;

                //影
                B->Style = bsSolid;
                B->Color = HalfColor(HalfColor(m_nFGColor, m_nBGColor, 0.5f), 0x0, 0.33f);
                C->FillRect(Rect(
                    leftmargin + textwidth + leftmargin * 2 + shadowoffset,
                    y + shadowoffset,
                    x + shadowoffset,
                ...
                C->FillRect(Rect(
                    leftmargin + textwidth + leftmargin * 2,
                    y,
                    x,
                    y + m_nFontHeight * 1.5
                    ));
        ...
        if (Card->m_nID != m_nTargetCard){
            ...
            if (contain){
                ...
                for (int i2 = 0 ; i2 < m_Document->m_Links->Count && !linked ; i2++){
                    TLink *Link = m_Document->GetLinkByIndex(i2);
                    linked |= Link->m_nFromID == m_nTargetCard && Link->m_nDestID == Card->m_nID;
                }
      ...
      TObject *Sender)
{
    ...
    if (Fo_Select->ModalResult == mrOk){
        ...
        for (int i = 0 ; i < Fo_Select->m_IDs->Count ; i++){
            TCardLabel *Label = m_Document->GetLabelByIndex(0, (int)Fo_Select->m_IDs->Items[i] - 1);
            if (i > 0){
                Card->m_Title += ", ";
            }
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    for (int i = 0 ; i < m_StatisticsRectToCard->Count ; i++){
        TSRectToCard *SRTC =  (TSRectToCard *)m_StatisticsRectToCard->Items[i];
        if (SRTC->m_Rect.Left <= X && SRTC->m_Rect.Right > X && SRTC->m_Rect.Top <= Y && SRTC->m_Rect.Bottom > Y){
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    if (Button == mbLeft){
        ...
        m_Drawing->MouseDown(
            (X - m_DrawingRect.left) * 1.0 / (m_DrawingRect.right - m_DrawingRect.left),
            (Y - m_DrawingRect.top) * 1.0 / (m_DrawingRect.bottom - m_DrawingRect.top),
...
//---------------------------------------------------------------------------
void TFo_Main::ExportHierarchicalText(TStringList *SL, int CurrentLevel, AnsiString HChar,
    TCard *CurrentParent){
...
void __fastcall TFo_Main::MS_OutputWordNgramClick(TObject *Sender)
{
    ...
    TTextDecomposer *TD = new TTextDecomposer(WS, n, 0, 0.0f, 0.0f, fdummy, bdummy, 0x1);

    //文字N-gramをファイルに出力
    FILE *F = fopen(SD_CSV->FileName.c_str(), "wt");
    ...
    while (true){
        ...
        int max = 0;
        int maxindex = 0;
        for (int in = n - 1 ; in >= 0 ; in--) if (currentindex[in] >= 0){
