...
//---------------------------------------------------------------------------
//高速Ini
TFastIniSection::TFastIniSection(AnsiString Name, TStringList *SL, int start, int end) :
    m_Name(Name),
    m_nLastIndex(-1)
{
...
int TFastIniSection::SearchKey(AnsiString Key)
{
...
AnsiString TFastIniSection::Values(int index)
{
    //return m_KeyValues->Values[index];
    int p = m_KeyValues->Strings[index].Pos("=");
    return m_KeyValues->Strings[index].SubString(p + 1, m_KeyValues->Strings[index].Length());
}
...
{
    int lastline = 0;
    AnsiString SectionName, NextSectionName;
    for (int i = 0 ; i < SL->Count ; i++){
        bool Split = false;
        if (SL->Strings[i].SubString(1, 1) == "["){
            NextSectionName = SL->Strings[i].SubString(2, SL->Strings[i].Length() - 2);
            Split = true;
        }

        if (Split){
            if (SectionName != ""){
                TFastIniSection *FIS = new TFastIniSection(SectionName, SL, lastline, i - 1);
                m_Sections->Add(FIS);
            }
    ...
    if (lastline < SL->Count){
        TFastIniSection *FIS = new TFastIniSection(SectionName, SL, lastline, SL->Count - 1);
        m_Sections->Add(FIS);
    }
...
//---------------------------------------------------------------------------
TFastIniSection *TFastIni::SearchSection(AnsiString Section)
{
...
AnsiString TFastIni::ReadString(AnsiString Section, AnsiString Key, AnsiString Default)
{
...
int TFastIni::ReadInteger(AnsiString Section, AnsiString Key, int Default)
{
    TFastIniSection *FIS = SearchSection(Section);
    if (FIS){
...
bool TFastIni::ReadBool(AnsiString Section, AnsiString Key, int Default)
{
    TFastIniSection *FIS = SearchSection(Section);
    if (FIS){
        return FIS->ReadInteger(Key, Default);
...
float TFastIni::ReadFloat(AnsiString Section, AnsiString Key, float Default)
{
    TFastIniSection *FIS = SearchSection(Section);
    if (FIS){
...
void TFastIni::ReadSectionValues(AnsiString Section, TStrings *S)
{
    S->Clear();
    TFastIniSection *FIS = SearchSection(Section);
    if (FIS){
...
{
    AnsiString S = "0000000" + IntToStr(i);
...
//---------------------------------------------------------------------------
AnsiString ForFileName(AnsiString S)
{
    AnsiString XStr = "\/:*?\"<>|";
...
//---------------------------------------------------------------------------
AnsiString DeleteActionKey(AnsiString S)
{
